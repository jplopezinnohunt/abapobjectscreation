...
def init_db():
    ...
    cursor.execute('CREATE TABLE IF NOT EXISTS fund_centers (FIKRS TEXT, FICTR TEXT, ERFDAT TEXT, ERFNAME TEXT, PRIMARY KEY (FIKRS, FICTR))')
    ...
    # Custom UNESCO tables
    cursor.execute('CREATE TABLE IF NOT EXISTS ytfm_fund_cpl (FIKRS TEXT, FINCODE TEXT, ALINE TEXT, NONIBF TEXT, PRIMARY KEY (FIKRS, FINCODE))')
    cursor.execute('CREATE TABLE IF NOT EXISTS ytfm_wrttp_gr (WRTTP_GRP TEXT, SEQNR TEXT, WRTTP TEXT)')
...
def sync_table(rfc, db_conn, sap_table, db_table, fields, options="", batch_size=2000):
    cursor = db_conn.cursor()
    skip = 0
    total_synced = 0
    print(f"--- Syncing {sap_table} -> {db_table} ---")
    ...
    print(f"Finished {sap_table}: {total_synced} rows total.")

def sync_movements(rfc, db_conn):
    cursor = db_conn.cursor()
    # Focus on UNES/UBO activity for current years
    for year in ["2024", "2025", "2026"]:
        opt = f"GJAHR = '{year}' AND ( FIKRS = 'UNES' OR FIKRS = 'UBO' )"
        print(f"Syncing movements for {year}...")
        
        # We use a larger batch for movements but we summarize in-memory to keep DB lean
        skip = 0
        summary = {}
        while True:
            res = rfc.call("RFC_READ_TABLE",
            ...
            if len(data) < 5000:
                break
            skip += 5000
            print(f"  [FMIFIIT {year}] Processed {skip} rows...")
        ...
        db_conn.commit()
        print(f"  [FMIFIIT {year}] Committed {len(summary)} unique combinations.")

def main():
    start_time = time.time()
    print(f"P01 Full Database Extraction Started: {datetime.now()}")
    
    try:
        rfc = get_p01_conn()
        db = init_db()
        
        # 1. Standard Master Data
        sync_table(rfc, db, "FMFINCODE", "funds", ["FIKRS", "FINCODE", "TYPE", "ERFDAT", "ERFNAME"], options=f"FIKRS IN {FM_AREAS}")
        sync_table(rfc, db, "FMFCTR", "fund_centers", ["FIKRS", "FICTR", "ERFDAT", "ERFNAME"], options=f"FIKRS IN {FM_AREAS}")
        
        # Projects and WBS - Getting a batch for 'UNESCO' company code
        sync_table(rfc, db, "PROJ", "projects", ["PSPID", "POST1", "VERNR", "VBUKR", "ERDAT"], options="VBUKR = '1000' OR VBUKR = 'UNESCO'")
        sync_table(rfc, db, "PRPS", "wbs_elements", ["POSID", "POST1", "PSPHI", "ERDAT"], options="PBUKR = '1000' OR PBUKR = 'UNESCO'")

        # 2. Custom UNESCO Config/Bridge
        sync_table(rfc, db, "YTFM_FUND_CPL", "ytfm_fund_cpl", ["FIKRS", "FINCODE", "ALINE", "NONIBF"], options=f"FIKRS IN {FM_AREAS}")
        sync_table(rfc, db, "YTFM_WRTTP_GR", "ytfm_wrttp_gr", ["WRTTP_GRP", "SEQNR", "WRTTP"])
        
        # 3. Movements Summary
        sync_movements(rfc, db)
        
        db.close()
        rfc.close()
        print(f"Sync Successful! Total duration: {time.time() - start_time:.2f} seconds.")
