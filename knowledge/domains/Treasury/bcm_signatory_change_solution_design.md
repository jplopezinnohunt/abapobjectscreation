...
**"Simulate rule resolution" — where its logic is defined.** The OOCU_RESP simulation has no logic of its own: you pass a runtime container (Key date + `Paying co. code` + `M.PymtAmt(rcur)`) and it evaluates that against each responsibility's criteria, returning the matching node's agents. Verified live (rule 90000004, co.code UBO, amount 100) → AMARAL/BA/NOLETO JOVCHELOVITCH… = node 50034894. **The logic is NOT a program/exit — it is the criteria on each responsibility**, seen by double-clicking the responsibility in OOCU_RESP (`Paying co. code = UBO` + `M.PymtAmt 0–10000`) = infotype 1218 (`HRP1218/HRT1218`). The result includes BA (10005016) who has no BNK_APP role (§3e) → **agent determination ≠ authorization**.
...
**FI substitutions (posting path, parallel to the block):** `T001Q` assigns per company code+callup → UNES: UNESCH(0001)+UNESCO(0002) · UBO: UNESCH+UBO · UIS:UIS · IIEP:IIEP · UIL:UIL (all active). Defined in `GGB1`/`OBBH`, exit program `YRGGBS00` (forms U100–U917), defs in `GB92`. **Verified: they do NOT set the payment block** (no `ZLSPR` in YRGGBS00) — separate customization.

**Where to see/maintain (tx · program · table):** doc subst `OB28`/`GGB1`/`OBBH` (`T001Q`,`GB92`) · block-W `SWDD` WS90000003 → BOR `YBSEG`/prog `YBSEG_REL` (`BSEG/BSIK.ZLSPR`) · block reason `OB27` (`T008T`) · F110/FBZP (`T042A`) · grouping IMG (`TBNK_RULE/_SELOP`) · batch `BNK_MONI` (`BNK_BATCH_HEADER`) · procedure IMG BCM Approval (`TBCA_RELPROC_CUS/_EXP`,`TBCA_REL_RULE`) · node `OOCU_RESP` (the criteria + people — use this). PP01 object type = **"Responsibility"** (=`RY`, it IS in the dropdown under "R") but lists only PD infotypes (Object/Relationships/Description/Profiles) — it does **NOT** show the selection criteria; those are in `HRP1218`/`HRT1218` (read via OOCU_RESP or RFC). People in `HRP1001`. Resolves via `RH_GET_ACTORS` · auth `PFCG`/`BNK_APP` (`AGR_USERS/AGR_TCODES`) · reject BAdI `BNK_BADI_ORIG_PAYMT_CHG` · output `DMEE`.
...
### 4c. Where the criteria are defined (OOCU_RESP overview) + dev/prod caveat
