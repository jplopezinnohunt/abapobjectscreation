...
Example: 9H/2026/UNES/3110111021/PAX/TC — ROBJNR ...3870146 (budget) = 4,500.00 USD; ROBJNR ...3870148 (consumption) = 547.65 USD → **available = 3,952.35 USD** (claims #304, #305).
...
**ALLOCTYPE_9** (data element BUAVC_ALLOCTYPE) domain values: KBFC=Hard commitments/allocated budget; SEEC/SENC/REEC/RENC/RSEC/RSNC = sender/receiver cover-element legs; ACCG/RIBC/ARIB/RIBL = cover-group legs. For **all 9 UNESCO FM areas KBFC is the only value in use** (FY2024-2026, 48,543 rows — claim #306). UNESCO does not use cover-element or cover-group splitting.
...
- ~~Our Gold-DB `fmavct_*` slice kept only ALLOCTYPE_9=KBFC+HSL01~~ — **REMEDIATED 2026-06-30**: Gold DB `fmavct_2024/2025/2026` now carry 38 columns including the full natural key. The prior 7-col schema collided ~36% of rows and could not resolve budget vs consumed (claims #304).

## Gold DB / Pipeline — AVC layer (added 2026-06-30)
The AVC layer is now part of the **recurring PSM_FM gold pipeline** (claim #307):
...
**RFC extraction pattern** for wide FMAVCT (claim #308): field-split into <=7-col groups, RFC-read each group, recombine by row position with an equal-rowcount guard. Necessary because RFC_READ_TABLE has a 512-byte WA buffer. P01 rejects ROWSKIPS (claim #244) — use ROWCOUNT=0 partitioned by RFIKRS x RYEAR.
...
Claims: #304 (77 fields / 38-col schema), #305 (ROBJNR-pair available formula), #306 (ALLOCTYPE_9 values + KBFC-only for all 9 areas), #307 (AVC layer pipeline registration), #308 (field-split RFC pattern).
