...
| **PPDIX** | `PPDIX` | Payroll posting **run index** | Joins a payroll RUN to its document lines: `RUNID`+`LINUM` → `DOCNUM`+`DOCLIN`. The middle link of the chain PPOIX → PPDIX → PPDIT. **PURGED s098 to posted runs only.** | EVTYP+RUNID+LINUM | 444,886 (2026, posted runs) |
| **PPOIX** | `PPOIX` | Payroll posting **index by employee** | The link that CLOSES the chain: carries `LGART` (wage type), `BETRG`+`WAERS` (the amount) and `KOMOK` (the CHAR(4) symbolic account) on one row — which `PPDIT` does not have. `TSLIN` joins to `PPDIX.LINUM`. Also carries four customer fields `ZZSUBST_*`: a local substitution of the symbolic account, always **from `SUSP`** (suspense), 1,264 times in 2026 on one to three employees a month; `ZZSUBST_AUART` and `ZZSUBST_SPPRC` are populated on zero rows. **PURGED s098 to posted runs only.** | PERNR+SEQNO | 1,302,607 (2026, posted runs) |
| **payroll_runs_posted** | *(VIEW, not a SAP table)* | The runs that produced an accounting document | `BKPF.AWKEY = PPDHD.DOCNUM AND AWTYP='HRPAY'`. **Any payroll amount must be read through this or simulations are counted as real** — 1,828 of 2,316 runs in 2026 never reached FI, and ignoring that overstated the Constant Dollar impact NINEFOLD. A run transfers whole or not at all; zero runs are partial. Only valid where BKPF coverage exists (2024-2026). | RUNID | 1,364 runs |
| **PPDIT** | `PPDIT` | Payroll posting document **ITEMS** | **THE ANSWER to the account determination.** Carries `KTOSL` (FI transaction key, CHAR(3) — HRA/HRC/HRF/HRK) AND `HKONT` (resolved GL account) on the same row, plus `FISTL`/`GEBER` (the FM assignment) and the custom `ZZWAERS`/`ZZBETRG`. Measured: a transaction key fans out to several GL accounts while every GL account belongs to exactly one key ⇒ the account is decided BEYOND the key, by the wage type. Holds the Constant Dollar 999 accounts, which net to **exactly zero** per currency. **PURGED s098 to posted runs only** — see `payroll_runs_posted`. | DOCNUM+DOCLIN | 434,681 (2026, posted runs) |
