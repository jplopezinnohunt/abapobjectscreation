...
def write_case_tab(wb, case):
    ...
    run_order = [
        "⚠ KEY CONSTRAINT — tx DMEE menu only exposes 'Format tree → Test active version' (F8). It tests ONLY whichever version has EX_STATUS='A'. To test V001 with the SAP engine, V001 MUST be activated first (which makes V001's content the new V000 per SAP-correct lifecycle in version-concept tab). The V001 test is therefore done in D01 with snapshot+activate+test+rollback procedure — NOT in P01.",
        "",
        "═══ PHASE A · Test V000 (current production behaviour) ═══",
        "STEP 1 — Open tx DMEE → Tree type=PAYM + Format tree=/SEPA_CT_UNES → click Display",
        "STEP 2 — In popup, click 'Active version' → tree opens in V000",
        "STEP 3 — Menu: Format tree → Test active version (F8) → 'DMEE Test Data' screen opens",
        "STEP 4 — Copy FPAYHX row from this Excel (skip 'Row' column) → FPAYHX grid",
        "STEP 5 — Copy FPAYH row → FPAYH grid (Doc cat. value matters: 01 vs 03)",
        "STEP 6 — Copy FPAYP row → FPAYP grid",
        "STEP 7 — Optional: expand FPAYHX layout to add Z-fields (ZLISO, ZPFST, ZPLOR, ZSTRA) — see Z-FIELDS section below. Without them, Cdtr/PstlAdr renders empty.",
        "STEP 8 — Click Test/Run → SAP renders V000 XML output",
        "STEP 9 — Save the V000 XML as 'case_<N>_V000.xml' (copy text or use Download XML file)",
        "",
        "═══ PHASE B · Test V001 (proposed design) — D01 ONLY, with snapshot+rollback safety ═══",
        "STEP 10 (D01 PRE-FLIGHT) — Confirm you are in D01 (NOT P01). V001 testing must NEVER be done in P01 because activation = overwrite of V000.",
        "STEP 11 (SNAPSHOT FOR ROLLBACK) — In D01: tx DMEE → /SEPA_CT_UNES → Display → Maintenance version → Format tree → Change → Utilities → Versions → Generate Version → SAP archives current D01 V000 to V002 (or next free number). Note the version number assigned. THIS IS THE ROLLBACK ENABLER — do not skip.",
        "STEP 12 (BUILD V001 IF NOT YET DONE) — In Change mode on V001, apply the matrix-row INSERTs/MODIFY/SYNC for the V001 design. Save → captures workbench transport (R3TR DMEE class K).",
        "STEP 13 (ACTIVATE V001) — Tree → Activate (or button on Change-mode toolbar). SAP runs consistency check; if clean, V001 content is stored as V000 (overwrites the previous V000 — but it's safely archived in V002 from STEP 11).",
        "STEP 14 (TEST THE NEW V000 = V001 CONTENT) — Repeat STEPS 1-9 above in D01. The 'Active version' is now what V001 used to be. The output XML is the V001 engine output for this input. Save as 'case_<N>_V001.xml'.",
        "STEP 15 (ROLLBACK D01 TO ORIGINAL V000) — Tree → Versions → Version Management → select V002 → Retrieve → V002 content copies into V001 → Activate → original V000 content restored as Active. D01 is now back to the pre-test state.",
        "",
        "═══ PHASE C · Diff and validate ═══",
        "STEP 16 — Diff case_<N>_V000.xml vs case_<N>_V001.xml. V001 should show ADDITIONAL <StrtNm>/<BldgNb>/<PstCd>/<TwnNm> tags inside <Cdtr><PstlAdr> and <Dbtr><PstlAdr>. If those new tags appear with the same input → V001 design empirically validated against the SAP engine for this case.",
        "STEP 17 — Compare both outputs against the Python simulator's predictions (Layer 1) for the same input. If V000 engine ≡ V000 simulator AND V001 engine ≡ V001 simulator → claim 105 (simulator correctness-equivalence) empirically proven for this case.",
    ]
    next_row = 4
...
def write_overview_tab(wb):
    ...
    rows = [
        ...
        ("", 10, False, "000000"),
        ("⚠ KEY CONSTRAINT discovered 2026-05-01 — tx DMEE only tests Active version", 12, True, "B45309"),
        ("The 'Format tree → Test active version (F8)' menu in tx DMEE tests ONLY the Active version (the one with EX_STATUS='A'). There is no 'Test maintenance version' option in the standard menu.", 10, False, "1f2937"),
        ("Implication: each case in this workbook has TWO PHASES.", 10, False, "1f2937"),
        ("   Phase A — Test V000 (current production): runs anywhere (D01/V01/P01); F8 tests the production state directly. No risk to anything. This is the L1↔engine equivalence proof for the simulator V000 predictions.", 10, False, "475569"),
        ("   Phase B — Test V001 (proposed design): MUST be done in D01 ONLY because V001 testing requires activation, which OVERWRITES V000 in the system where activation runs. Procedure: (a) Generate Version snapshot of D01 V000 to V002 [rollback enabler], (b) Activate V001 in D01 [V001 content becomes the new V000 in D01], (c) F8 Test active version in D01 [now testing what was V001], (d) save the XML, (e) ROLLBACK by retrieving V002 from Version Management → Activate [original D01 V000 restored].", 10, False, "475569"),
        ("   NEVER do Phase B in P01. P01 stays on V000 production behaviour throughout Phase 3 testing.", 10, False, "1f2937"),
        ("Each case tab below has the explicit STEP-by-STEP procedure for both phases (17 steps total per case = 9 for V000 + 6 for V001 activation/test/rollback in D01 + 2 for diff).", 10, False, "475569"),
        ("", 10, False, "000000"),
