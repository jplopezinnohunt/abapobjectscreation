"""
abap_bridge_create_class.py
Use RFC_ABAP_INSTALL_AND_RUN to call SEO_CLASS_CREATE_COMPLETE locally in SAP.
The ABAP bridge resolves all type metadata locally, avoiding remote type resolution errors.
"""
import os, sys
...
for cls_name, cls_desc in CLASSES:
    # CRITICAL: No VALUE() constructors, no inline DATA declarations
    # Use flat declarations only, as required by RFC_ABAP_INSTALL_AND_RUN
    abap = f"""
...
ls_clasdesc-clsname = '{cls_name}'.
ls_clasdesc-version = '1'.
ls_clasdesc-langu   = 'E'.
ls_clasdesc-descript = '{cls_desc[:40]}'.
APPEND ls_clasdesc TO lt_clasdesc.

* Check existing state
SELECT SINGLE state FROM seoclassdf
  INTO lv_state
  WHERE clsname = '{cls_name}'.
WRITE: / 'Current SEOCLASSDF state:', sy-subrc, lv_state.
