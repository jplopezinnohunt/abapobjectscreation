"""
extract_derivation_config_session71.py
=======================================
Session #071 — Pull OKB9 (TKA3D/TKA3E) + FMDERIVE config tables to complete the
4-field derivation picture (BA, CC, Fund, FundCenter) for INC-000008088.
...
def main():
    print("\n  Derivation-config Extraction — P01 (Session #071)")
    print("  " + "=" * 60)
    ...
    target_tables = [
        # OKB9 — Default account assignment per cost element
        "TKA3D",   # OKB9 default account assignment per cost element (KOKRS+KSTAR -> KOSTL/PRCTR)
        "TKA3E",   # OKB9 with company code (KOKRS+BUKRS+KSTAR -> KOSTL)
        "TKA3F",   # OKB9 with profit center / business area
        "TKA3G",   # OKB9 default per business area
        # FMDERIVE strategy
        "T8JFB",   # FM Derivation Strategy - header
        "T8JFC",   # FM Derivation Strategy - step
        "T8JFD",   # FM Derivation Strategy - step values
        "T8JFFEAS","T8JFFFEAS",  # field assignments
        "RUDS_DRC","RUDS_DT","RUDS_DRD",  # Generic derivation tool tables
        # SKB1 — chart-specific G/L account properties (already noted not in Gold DB)
        "SKB1",    # GL Company-code-specific (VZSKZ, MWSKZ, etc.)
        "SKAT",    # GL Account texts
        # OB52 / OBC4 not relevant — focus on derivation
    ]
    ...
    for tbl in target_tables:
        ...
        if dd_fields:
            ...
            field_map[tbl] = names
            print(f"    {tbl:14s} -> {len(names):>3} fields: {', '.join(names[:8])}{'...' if len(names) > 8 else ''}")
        else:
            field_map[tbl] = []
            print(f"    {tbl:14s} -> NOT FOUND or no access")
        time.sleep(0.2)

    print(f"\n  Phase 2: Extract\n")
    # For SKB1 + SKAT we want to filter to BUKRS=UNES / KTOPL=UNES; others extract all
    where_clauses = {
        "SKB1": "BUKRS = 'UNES'",
        "SKAT": "KTOPL = 'UNES'",
        "TKA3E": "BUKRS = 'UNES'",
    ...
    for tbl in target_tables:
        if not field_map.get(tbl):
            print(f"  [{tbl:14s}] SKIP — no fields")
            results[tbl] = {"rows": 0, "status": "NOT_FOUND"}
            continue
        extract_fields = field_map[tbl][:8]
        where = where_clauses.get(tbl, "")
        print(f"  [{tbl:14s}] Extracting {len(extract_fields)}/{len(field_map[tbl])} fields, WHERE: {where or '(none)'}")
        try:
            rows = read_table_simple(conn, tbl, extract_fields, where=where)
            print(f"    -> {len(rows):,} rows")
            load_to_sqlite(db, tbl, extract_fields, rows)
            results[tbl] = {"rows": len(rows), "status": "OK", "fields": len(extract_fields)}
            # Expand to remaining fields
            if rows and len(extract_fields) < len(field_map[tbl]):
                ...
                for batch_start in range(0, len(remaining), 4):
                    ...
                    try:
                        rows2 = read_table_simple(conn, tbl, expanded, where=where)
                        if rows2:
    ...
    for tbl, info in results.items():
        print(f"    {tbl:14s} {info.get('status'):20s} rows={info.get('rows', 0)} fields={info.get('fields', '-')}")
