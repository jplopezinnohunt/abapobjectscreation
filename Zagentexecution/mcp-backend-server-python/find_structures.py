...
def find_structures(prefix):
    conn = get_conn()
    print(f"Searching for structures matching {prefix}...")
    try:
        r = conn.call("RFC_READ_TABLE", QUERY_TABLE="DD02L", DELIMITER="|",
                      OPTIONS=[{"TEXT": f"TABNAME LIKE '{prefix}%' AND TABCLASS = 'INTTAB'"}],
...
if __name__ == "__main__":
    find_structures("SEO_CLASS")
    find_structures("V_SEOCLASS")
    find_structures("VSEOCLASS")
