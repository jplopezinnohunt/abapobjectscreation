...
*       1-Update table of  request
        lo_util->update_req_table( EXPORTING  iv_guid = iv_guid
                                     iv_new_status = iv_status "c_req_submited_employee
                                     iv_next_actor = c_hra
...
*       3-Update history log
        lo_util->update_histo_table( EXPORTING iv_actor = c_employee
                                      iv_guid = iv_guid
                                      iv_new_status = iv_status "c_req_submited_employee
                                      iv_upd_first_name = lv_first_name
                                      iv_upd_last_name = lv_last_name
...
*       4-Update coment log
        IF iv_coment IS NOT INITIAL.
          lo_util->create_coment_for_request( EXPORTING iv_actor = c_employee
                                                   iv_coment = iv_coment
                                                   iv_date   = sy-datum
                                                   iv_guid   = iv_guid
                                                   iv_pernr  = lv_pernr
