"""Deploy Y_FI_DMEE_ADR v5 (employee detection via KTOKK='SCSA')."""
import os, sys
...
SRC = os.path.join(os.path.dirname(__file__), "..", "..",
    "extracted_code/FI/DMEE_full_inventory/Y_FI_DMEE_ADR_v5.abap")
INCLUDE = "LYFPAYMU19"

with open(SRC, encoding="utf-8") as f:
    src_lines = f.read().splitlines()
print(f"v5 source: {len(src_lines)} lines")
for marker in ["lv_is_employee", "WHEN 'Cdtr'", "WHEN 'Dbtr'", "ktokk", "REFERENCE(I_EXTENSION)"]:
    print(f"  marker '{marker}': {sum(1 for l in src_lines if marker in l)}")
...
r = c.call("RFC_ABAP_INSTALL_AND_RUN", MODE='F', PROGRAMNAME='ZFIX_YFI5',
