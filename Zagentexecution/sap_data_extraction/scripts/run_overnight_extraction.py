...
def print_status_report():
    ...
    for tbl, d in table_dirs.items():
        ...
        status   = "✓ COMPLETE" if len(files) >= expected else f"{pct}% done"
        overall_rows += total
...
def run_fi_pipeline(years, batch_size, throttle):
    log("Starting FI pipeline (BKPF + BSEG)", "FI")
    t0 = time.time()
    try:
        ...
        # Launch BKPF and BSEG threads — they will each acquire 1 semaphore slot
        threads = []
        for tbl, fields, date_f, out_d in [
            ("BKPF", fi.BKPF_FIELDS, "BUDAT", fi.BKPF_DIR),
            ("BSEG", fi.BSEG_FIELDS, "BUDAT", fi.BSEG_DIR),
        ]:
            t = threading.Thread(
            ...
            threads.append(t)
            t.start()
            log(f"{tbl} thread queued (waiting for semaphore slot)", "FI")
            time.sleep(0.5)
...
def main():
    ...
    parser.add_argument("--batch-size", type=int,   default=200,   help="Rows per RFC call (default: 200)")
    parser.add_argument("--throttle",   type=float, default=1.0,   help="Seconds between RFC pages (default: 1.0)")
