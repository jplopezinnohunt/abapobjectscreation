METHOD teamcalendarempl_update_entity.
...
* check if personalization is allowed
  ...
  IF lv_personalization_mode = if_hcmfab_tcal_constants=>gc_app_pers_mode-none.
    lv_param = er_entity-employee_id.
...
* change the display state to the new value
  me->change_employee_display_state(
    EXPORTING
      ...
      iv_display_state       = er_entity-display_state
  ).

* add sharing request if needed
* get approval mode
  ...
  IF lv_approval_mode <> if_hcmfab_tcal_constants=>gc_approval_mode-none
    AND er_entity-display_state = if_hcmfab_tcal_constants=>gc_display_state-requested.
    ls_shared_employee-mandt = sy-mandt.
    ls_shared_employee-employee_id = er_entity-employee_id.
    ls_shared_employee-shared_employee_id = er_entity-employee_assignment.
...
*   trigger approval workflow (if needed)
    IF lv_approval_mode = if_hcmfab_tcal_constants=>gc_approval_mode-workflow.
      CALL BADI go_badi_tcal_settings->trigger_approval
        EXPORTING
          iv_requester_pernr = er_entity-employee_assignment
          iv_approver_pernr  = er_entity-employee_id.
    ENDIF.
  ENDIF.
ENDMETHOD.
