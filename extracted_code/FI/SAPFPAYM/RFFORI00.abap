...
*----------------------------------------------------------------------*
* Common and hidden parameters                                         *
*----------------------------------------------------------------------*
PARAMETERS:
* if program is scheduled with SAPF110S/SAPF111S, then it shall wait for
* update task being finished if payment document validation is active
  par_f11s TYPE xfeld NO-DISPLAY.                              "n1922823
...
data: begin of tab_xml_avis occurs 10, "Avise, die per XML versendet
        reguh like reguh,              "wurden (Info für die Listaus-
        regud like regud,              "gabe im Avisformular)
      end of tab_xml_avis.             "advices sent by XML
...
data:
  ...
  flg_dd_prenotif(1)                   "1 - direct debit prenotifications
                    type n value 0,    "via program RFFOAVIS_DD_PRENOTIF
...
data:
  ...
  cnt_avxml(5)      type p,            "number of rem. advices via xml "EhP5
  ...
  cnt_advices_per_spool type i.        "number of advices per spool    "2384863
...
data:
  ...
  sum_abschl_xml(8)  type p,           "paymt amount summary via xml  "EhP5 Avis per XML
...
data:
  ...
  hlp_filename(255)  type c,            "file name (DME), note 2059675
  gd_filename_new(255) type c,          " alternative filename, note 1460101, note 2059675
  ...
  data ubnka_n      like bnka.  " note 1673302, for swift code of house bank
