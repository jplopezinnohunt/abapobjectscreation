...
* Fill application of run
  ...
  IF hlp_laufk = 'B'.                                     "Note 2899195
    ...
    SELECT SINGLE laufi laufd FROM reguhm INTO (l_laufi, l_laufd)
      WHERE laufi_m = zw_laufi AND laufd_m = zw_laufd.
...
* prepare authorization checks -----------------------------------------
  IF hlp_laufk NE '*'.
    IF hlp_laufk NE 'P'.               "Prüfung für FI-Bestände
      ...
      IF sy-subrc NE 0 AND flg_dd_prenotif NE 0.         "start n1941747
        AUTHORITY-CHECK OBJECT 'F_REGU_KOA'
          ID 'KOART' FIELD 'K'
          ID 'FBTCH' FIELD '16'.
      ENDIF.                                               "end n1941747
      ...
      IF flg_koart_auth EQ space.      "Keine Kontoarten-Berechtigung
                                       "no account type authority
        IF flg_dd_prenotif NE 0.                               "n1941747
          hlp_fbtch = 16.
        ENDIF.
        IF sy-batch EQ space.
...
* Read texts from textpool SAPDBPYF ------------------------------------
  PERFORM text(sapdbpyf) USING:
   ...
   099 text_099, 555 text_555,               816 text_816,
                                             820 text_820,
                                             821 text_821,
                                             822 text_822,
                                             830 text_830,
...
* Lastschriftankündigungen dürfen nur mit RFFOAVIS_DD_PRENOTIF ausgegeben
* Direct debit prenotifications can only be printed with RFFOAVIS_DD_PRENOTIF
  IF reguv-x_dd_prenotif EQ 'X' AND flg_dd_prenotif EQ 0.
    IF sy-batch EQ space.
      MESSAGE i477.
    ELSE.
      MESSAGE s477.
    ENDIF.
    STOP.
  ELSEIF reguv-x_dd_prenotif EQ space AND flg_dd_prenotif EQ 1.
    IF sy-batch EQ space.
      MESSAGE i478.
    ELSE.
      MESSAGE s478.
  ...
  IF NOT sy-repid = 'RFFOAVIS'.  " 1604671
...
* bei Sofort-Einplanung und Zahlungsbelegverprobung warten,
* bis die Zähler in REGUV gleich sind
* when payment program and payment media are scheduled together
* and payment document validation is active,
* then wait until counter in REGUV are equal
  IF NOT par_f11s IS INITIAL AND NOT par_belp IS INITIAL.      "n1922823
    PERFORM wait_for_validation IN PROGRAM sapfpaym
      USING zw_laufd
            zw_laufi
            zw_xvorl.
  ENDIF.
...
* authorization checks -------------------------------------------------
  IF hlp_laufk NA 'P*'.                "Prüfung für FI-Bestände
    ...
    IF sy-subrc NE 0.
      ...
      IF flg_dd_prenotif EQ 0.                           "start n1941747
    ...
    IF flg_dd_prenotif NE 0.                             "start n1941747
      err_auth-actvt = hlp_fbtch.
    ELSE.
      err_auth-actvt = 16.
    ENDIF.                                                 "end n1941747
...
* Mandat-Referenz
  DATA:
    l_mandate_key  TYPE sepa_str_mandate_key,
    lt_mandate     TYPE sepa_tab_data_mandate_data.
  FIELD-SYMBOLS:
    <ls_mandate>   TYPE sepa_str_data_mandate_data.

  CLEAR:
    regud-mndid,
    regud-crdid.
  IF NOT reguh-mguid IS INITIAL.
    l_mandate_key-mguid = reguh-mguid.
    CALL FUNCTION 'SEPA_GET_MANDATE_BY_KEY'
      EXPORTING
        i_str_mandate_key = l_mandate_key
      IMPORTING
        et_mandates       = lt_mandate.
    READ TABLE lt_mandate
      WITH KEY mguid = reguh-mguid
      ASSIGNING <ls_mandate>.
    IF sy-subrc EQ 0.
      regud-mndid = <ls_mandate>-mndid.
      regud-crdid = <ls_mandate>-rec_crdid.
    ENDIF.
  ENDIF.
...
DATA: dref_xblnr TYPE REF TO data.                        "note 1725723
FIELD-SYMBOLS: <xblnr>.                                   "note 1725723
...
*    So, removed the check.
  ...
  IF bkpf-awsys EQ t000-logsys OR bkpf-awsys IS INITIAL.
    ...
    ELSE.
      ...
      READ TABLE l_import_tab                             "note 1725723
        WITH KEY parameter = 'XBLNR'.                     "note 1725723
      CREATE DATA dref_xblnr TYPE (l_import_tab-typ).     "note 1725723
      IF sy-subrc = 0.                                    "note 1725723
        ASSIGN dref_xblnr->* TO <xblnr>.                  "note 1725723
      ENDIF.                                              "note 1725723
      IF NOT <xblnr> IS ASSIGNED.                         "note 1725723
        ASSIGN regup-xblnr TO <xblnr>.                    "note 1725723
      ENDIF.                                              "note 1725723
      <xblnr> = regup-xblnr.                              "note 1725723
...
*** Func found so check its import parameters.
      ...
      ENDIF.                                              "note 1725723
      IF NOT <xblnr> IS ASSIGNED.                         "note 1725723
        ASSIGN tab_rfc-xblnr TO <xblnr>.                  "note 1725723
      ENDIF.                                              "note 1725723
      <xblnr> = tab_rfc-xblnr.                            "note 1725723
...
*       remove the reject: we are not in get event.    "  3135243
*       the reject causes the program to stop, without display
*       of messages and logs.
*        REJECT.   "  3135243
      ENDIF.
      IF sy-subrc EQ 8.
*     store error message from HR
        IF sy-msgid = 'HR3PRNA' and sy-msgno = 310."note 1737771
*        "suppress this message.not true exception."note 1737771
        ELSE.                                      "note 1737771
        fimsg-msgv1   = sy-msgv1.
        fimsg-msgv2   = sy-msgv2.
        fimsg-msgv3   = sy-msgv3.
...
*  if advice for BCM run, check if it is HR/HCM run
    IF reguh-laufi+5(1) = 'B'.
      DATA: l_laufi   LIKE reguhm-laufi_m,
            l_laufd   LIKE reguhm-laufd_m,
            l_laufi_o LIKE reguhm-laufi_m,
            l_laufd_o LIKE reguhm-laufd_m.
*    get value of original payment run
      DATA ls_reguhm LIKE reguhm.
      clear ls_reguhm.
      SELECT SINGLE * from reguhm INTO ls_reguhm
         WHERE laufi_m = reguh-laufi AND laufd_m = reguh-laufd
*          we use regup-zbukr because reguh-zbukr is cleared
*          because of sorting, see sic_reguh
           AND zbukr = regup-zbukr
           AND lifnr = reguh-lifnr AND
               kunnr = reguh-kunnr AND empfg = reguh-empfg AND
               vblnr = reguh-vblnr.
      SELECT SINGLE laufi laufd FROM reguhm INTO (l_laufi_o, l_laufd_o)
          WHERE laufi_m = ls_reguhm-laufi AND laufd_m = ls_reguhm-laufd
            AND zbukr   = ls_reguhm-zbukr AND lifnr   = ls_reguhm-lifnr AND
                kunnr   = ls_reguhm-kunnr AND empfg   = ls_reguhm-empfg AND
                vblnr   = ls_reguhm-vblnr.
      IF sy-subrc = 0.
*       call FM with original *P payment run
      ...
      ELSE.
         sy-msgid = 'F0'.
         sy-msgv1 = 'Error REGUHM include RFFORI99  VBLNR='.
         sy-msgv2 = reguh-vblnr.
         sy-msgno = 065.
         sy-msgty = 'E'.
      ENDIF.
    ELSE.
  ...
  LOOP AT err_nicht_verbucht.
    ...
    AT FIRST.
      ...
      IF reguv-x_dd_prenotif IS INITIAL.
        ...
        PERFORM message USING '256'.
      ELSE.
        PERFORM message USING '487'.
*       PERFORM message USING '273'.                        "n1839982
        PERFORM message USING '491'.
      ENDIF.
    ENDAT.
...
FORM datei_oeffnen.
  ...
  ELSE.                                "disk-/tape-fmt on file-system
    ...
    if gd_filename_new is initial.  " 1460101
      ...
      hlp_filename+243 = cnt_filenr. "note 2059675
    else.
...
* begin 1918654
    CALL FUNCTION 'FILE_VALIDATE_NAME'
      ...
      CHANGING
        physical_filename = hlp_filename
      EXCEPTIONS
        OTHERS            = 1.
    ...
    ELSE.  " unicode system
      ...
      ELSEIF p_codep = '4110'.   " UTF-8
        OPEN DATASET hlp_filename IN TEXT MODE FOR OUTPUT
                                          ENCODING DEFAULT
                                          IGNORING CONVERSION ERRORS. "1677415
...
*           I_WBGRU            = ' '
  ...
  IF sy-subrc NE 0.
    SELECT * FROM t042v WHERE bukrs EQ reguh-zbukr
                          ...
                          ORDER BY PRIMARY KEY.   " note 1793864
      EXIT.
    ENDSELECT.
    IF sy-subrc EQ 0.
