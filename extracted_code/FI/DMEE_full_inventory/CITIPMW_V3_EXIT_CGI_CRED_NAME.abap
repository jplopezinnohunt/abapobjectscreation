FUNCTION /citipmw/v3_exit_cgi_cred_name.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
  ...
  IF l_fpayh-gpa1t EQ '14'.
    ...
    IF sy-subrc EQ 0.
      IF NOT ls_xbsec IS INITIAL .
        MOVE ls_xbsec-name1 TO lv_cred_name.
      ENDIF.
    ENDIF.
  ELSE.

    CLEAR : v_vblnr.
    IF l_fpayhx-ctgypurp = 'TRAD'.
      IF l_fpayh-xvorl IS INITIAL.
        CONCATENATE 'P' l_fpayh-doc1r+1(9) INTO v_vblnr.
      ELSEIF l_fpayh-xvorl = 'X'.
        v_vblnr = l_fpayh-doc1r+4(10).
      ENDIF.

      SELECT SINGLE adrnr FROM reguh INTO wa_addrnr
             WHERE zbukr = l_fpayh-zbukr
             AND   laufd = l_fpayh-laufd
             AND   laufi = l_fpayh-laufi
             AND   xvorl = l_fpayh-xvorl
             AND   vblnr = v_vblnr.
    ENDIF.

    IF sy-subrc NE 0 OR wa_addrnr IS INITIAL.
...
*        READ TABLE itab_adrc INTO wa_adrc WITH KEY nation = i_uparam+213(1).
        ...
        IF NOT wa_adrc IS INITIAL.
          MOVE wa_adrc-name1 TO lv_cred_name.
        ENDIF.
      ENDIF.
    ENDIF.
