FUNCTION /citipmw/v3_exit_cgi_cred_city.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
  ...
  IF l_fpayh-gpa1t EQ '14'.
    ...
    IF sy-subrc EQ 0.
      IF NOT ls_xbsec IS INITIAL .
        MOVE ls_xbsec-ort01 TO lv_cred_city.
      ENDIF.
    ENDIF.
  ELSE.
...
*        READ TABLE itab_adrc INTO wa_adrc WITH KEY nation = i_uparam+213(1).
        ...
        IF NOT wa_adrc IS INITIAL.
          MOVE wa_adrc-city1 TO lv_cred_city.
        ENDIF.
      ENDIF.
    ENDIF.
  ENDIF."One time vendor check
...
*For F111 payments FPAYP-DOC2T = '05'
  IF ( l_fpayh-dorigin = 'FI-AP-PR' OR l_fpayp-doc2t = '05' OR l_fpayp-doc2t = '03' ) AND lv_cred_city IS INITIAL.
    lv_cred_city = l_fpayh-zort1.
  ENDIF.

  c_value = lv_cred_city.
