...
form interactive_display using sel    type c
  ...
  call function 'PTRV_SELECT_DISPLAY_OBJECT'
    exporting
      ...
      start_row        = 10
      start_column     = 20
      p_ldbtoday       = ldbtoday                          "ARIK029401
    exceptions
      no_buttons       = 1
      others           = 2.
...
*AAHOUNOU04112013
    <ihead>-zzsector = ipernr_tmp-zzsector.
    <ihead>-retxt = ipernr_tmp-retxt.
...
*&---------------------------------------------------------------------*
*&      Form  filter_trips_sa_ihead
*&---------------------------------------------------------------------*
*       filter trips and/or separation-allowances in table ihead
*----------------------------------------------------------------------*
*       MZCK001201
*----------------------------------------------------------------------*
FORM filter_trips_sa_ihead.
  IF sa_excl IS INITIAL. "for all trips & sep. allowances
    PERFORM delete_8xx_ihead.
  ENDIF.

  CHECK sa_all IS INITIAL. "no filter needed

  LOOP AT ihead.
    IF sa_excl IS INITIAL . "delete trips
      CHECK ihead-perio = 0.
    ENDIF.
    IF sa_incl IS INITIAL. "delete separation-allowances
      CHECK ihead-perio > 0.
    ENDIF.
    DELETE ihead.
  ENDLOOP.

ENDFORM.                    " filter_trips_sa_ihead
*&---------------------------------------------------------------------*
*&      Form  delete_8xx_ihead
*&---------------------------------------------------------------------*
*       delete sep. allowances with perio >= 800
*----------------------------------------------------------------------*
*       MZCK001201
*----------------------------------------------------------------------*
FORM delete_8xx_ihead.
  LOOP AT ihead WHERE perio >= 800.
    DELETE ihead.
  ENDLOOP.
ENDFORM.                    " delete_8xx_ihead
