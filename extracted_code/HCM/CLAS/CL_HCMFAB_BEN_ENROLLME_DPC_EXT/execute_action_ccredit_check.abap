METHOD execute_action_ccredit_check.
  DATA:lt_check_error TYPE hrben00err_ess_det_consistency,
     lt_costcred_check_error TYPE hrben00err_ess_det_consistency,
  ...
  CLEAR: ls_processed_plan,lv_event,lv_etype,lt_check_error,lt_error_table,lv_subrc.
  LOOP AT it_parameter INTO ls_param.
    CASE ls_param-name.
      ...
      WHEN 'AddnlInsCovUnit'.
        ls_costcred_trans-addun = ls_param-value.
      WHEN 'AddnlInsCovNum'.
        ls_costcred_trans-addno = ls_param-value.
      WHEN 'AddnlInsCovUnitMax'.
        ls_costcred_trans-addma = ls_param-value.
      WHEN 'AddnlInsCovUnitMin'.
        ls_costcred_trans-addmi = ls_param-value.
      WHEN OTHERS.
    ENDCASE.
  ENDLOOP.
  lv_datum = sy-datum.
...
* Fill global tables
      CALL FUNCTION 'HR_BEN_ESS_GET_OFFER'
        ...
        IF lv_subrc EQ 0.
* additional units
        check_additionalunits(
          EXPORTING
            iv_costcred_trans = ls_costcred_trans
            iv_processed_plan = ls_processed_plan
            iv_selected_enroll_reason = ls_selected_enroll_reason
          IMPORTING
            et_costcred_check_error = lt_costcred_check_error
            et_error_table = lt_error_table
          ).
        ENDIF.
        IF lt_costcred_check_error IS NOT INITIAL.
          LOOP AT lt_costcred_check_error INTO ls_check_error.
