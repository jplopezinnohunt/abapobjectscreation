...
FUNCTION ptrv_acc_employee_exp_post.
*"----------------------------------------------------------------------
*"*"Lokale Schnittstelle:
*"  IMPORTING
*"     VALUE(DOCUMENTHEADER) LIKE  BAPIACHE04 STRUCTURE  BAPIACHE04
...
*"  TABLES
*"      ACCOUNTGL STRUCTURE  BAPIACGL04
*"      ACCOUNTTAX STRUCTURE  BAPIACTX01 OPTIONAL
...
*"----------------------------------------------------------------------
  ...
  IF NOT unique_receiver IS INITIAL AND lv_use_srfc IS INITIAL.
    ...
    CALL FUNCTION 'ALE_ACC_EMPLOYEE_EXP_POST'
      EXPORTING
  ...
  ELSE.
    ...
    CALL FUNCTION 'BAPI_ACC_EMPLOYEE_EXP_POST' DESTINATION unique_receiver-rfc_dest
      EXPORTING
        documentheader        = documentheader
      TABLES
        accountgl             = accountgl
