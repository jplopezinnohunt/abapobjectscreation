...
FUNCTION ptrv_acc_employee_rec_post.
*"----------------------------------------------------------------------
...
*"  TABLES
*"      ACCOUNTRECEIVABLE STRUCTURE  BAPIACAR05
...
*"----------------------------------------------------------------------
  ...
  DATA: unique_receiver      LIKE bdbapidest,
        receivers            LIKE bdi_logsys OCCURS 1 WITH HEADER LINE,
        filterobjects_values LIKE bdi_fobj OCCURS 1 WITH HEADER LINE.
  ...
  IF NOT unique_receiver IS INITIAL AND lv_use_srfc IS INITIAL.
    ...
    CALL FUNCTION 'ALE_ACC_EMPLOYEE_REC_POST'
      EXPORTING
  ...
  ELSE.
    ...
    CALL FUNCTION 'BAPI_ACC_EMPLOYEE_REC_POST' DESTINATION unique_receiver-rfc_dest
      EXPORTING
        documentheader        = documentheader
      TABLES
        accountreceivable     = accountreceivable
