...
12. **Never read FF67 as configuration — neither its header nor its account list.** The header is `FEBKO-ABSND`, the identity carried by the last imported file: after an account number change it keeps showing the old value forever, and that is correct. **And the account list is statement HISTORY too** — proved 2026-08-28: it offers the pair `(SP0000000MX7, UNO10)`, which does **not** exist in `T012K` (NTB01 uses `SP0000000MXL` today) but does exist in `FEBKO.ABSND` with 10 statements last seen **2015-03-05**. A configuration-derived list cannot produce that. **Consequence: a newly created account never appears in FF67 until its first statement lands.** When a user says "the new account is not in FF67", do not go check the house bank record — check whether any statement has arrived.
...
**Classify by BEHAVIOUR, not by name — and prefer it.** `bank_account_behaviour_signature.py` scores every account on three axes over 2025-2026: pays (`REGUH`), receives statements (`FEBKO`/`EFART`), moves balance (`GLT0` periods with movement). It explains **130 of 144** accounts and leaves 14 that genuinely need explaining — better questions than the text gives. Three anomalies worth knowing: the **three Northern Trust mandates MOVE money and receive no statement at all** (a control gap; the fourth, IMIP, is truly dormant — so the four are not one block); **Beijing x2 and Bangkok take 1,483 statements with zero movement on the 10xxxxx G/L** in two years; and **58 accounts show payment activity while only 12 are in the automatic bank determination**. Also: of the accounts the text cannot classify, **102 are already declared as field-office by their `YBANK_..._FO_*` set** — check the set before concluding 'unclassified'.
