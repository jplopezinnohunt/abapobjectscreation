...
| **5 · boundary** | the integration map is built from `RFCDES`/`EDIDC`/`TBTCO`/`TBTCP`/`ICFSERVICE` extracts · `.claude/skills/sap_interface_intelligence` · `.claude/skills/sap_job_intelligence` |
...
| **7 · S/4 readiness** | **`brain_v2/build_s4_readiness.py`** (the cross-cutting DIMENSION as a subject, not a column — per-domain factors made queryable) · `brain_v2/capability_model/s4_readiness_model.json` (the verified factor method) · `.claude/skills/sap_change_audit` |
...
| **8 · transport** | `.claude/skills/sap_transport_intelligence` · CTS extracts (`cts_transports`, `cts_objects`) |
...
| **operation, derived** | **`process_mining/interface_boundary.py`** (F1 — configured vs observed; DEAD and UNDECLARED are the findings) · **`process_mining/derive_satellites.py`** (F2 — group endpoints by call signature to recover a GUID fleet) · **`process_mining/detect_drift.py`** (A7 — concept drift over accumulated history; per-day RATES, never raw monthly volumes) · **`process_mining/derive_object_roles.py`** (C4 — what an object is FOR, not merely where it belongs) · **`process_mining/caller_parse.py`** (one parser for the audit caller string, plus the truncation reconciliation) · **`process_mining/attach_object_text.py`** (the human name of every object) · **`process_mining/attribute_changes_to_programs.py`** (A8 — what WRITES a thing and through which channel; see the section below) · **`process_mining/chain_lineage.py`** **`process_mining/change_governance.py`** (A17 — CHANGE GOVERNANCE: channel difference, configuration never transported, and transported DATA. Needs no policy document — the population of maintainers defines the norm) · **`process_mining/payroll_discovery.py`** (A16 — PAYROLL END TO END: engine, logic, output, gates, master data and POSTING, with the enhancements on the posting path. Built on the premise that payroll logic is named after what it PRODUCES, so it searches from the output backwards) · **`process_mining/reality_filter.py`** (A18 — REALITY FILTER: measures the distance between what the system RECORDS and what HAPPENED, both directions — BACKWARD for recorded rows that never became real (simulations, reversals, parked) and FORWARD for configured entries that never produce a document (dormant design). Fires BEFORE summing any operational table for a reported figure and before calling a configured mechanism THE mechanism; born s098 when the same payroll subject produced both errors on the same day — the staff budget-rate impact reported at USD 20.5M was USD 1.98M (90% simulation runs), and 72 configured Constant Dollar wage types were described as the mechanism when zero of them post) · **`brain_v2/build_br_graph.py`** (A15 — SUBJECT GRAPH: relates the pieces of a subject by typed edges, each node pointing at where its content lives. Gates on unresolved content, orphan nodes and dangling edges — a conclusion with no edges is prose with a JSON wrapper) · **`process_mining/br_impact.py`** (A14 — BUDGET-RATE IMPACT: replicates the organisation's own four baselines rather than reconstructing a definition from outside; reports per route because one route holds the lines and another holds the money) · **`process_mining/harvest_custom_fields.py`** (A13 — CUSTOM FIELD HARVEST: every customer-namespace extension to the standard model, how it is used, and whether anything is in it. The EMPTY ones are the point — extension points that already exist and already survived every upgrade) · **`process_mining/traverse_hierarchy.py`** (A12 — SET HIERARCHY TRAVERSAL: walks a set class to every leaf and reports ORPHANS, within-tree repetition and cross-tree versioning BEFORE any amount, because a roll-up total is unusable until those are known. Identify the class by traversal, never by SAP convention or by value match) · **`process_mining/chain_lineage.py`** (A10 — the ADDRESS CHAIN: how funding reaches work when no foreign key says so. Spec-driven via `chain_spec.json`; emits LIVE / SUBSTITUTED / COLLAPSED / UNEVEN / SPARSE / BROKEN per hop) |
