...
4. **CODE TRACE** — check `extracted_code/`. If a needed file is missing, run `sap_adt_api` BEFORE continuing. Cite `file.abap:line` for every claim. **If the symptom is a blocked posting (ZFI nnn), a field that changed by itself, or unexpected exit behavior: load the `sap_validation_substitution` skill (the whole GGB0/GGB1/YRGGBS00 perimeter + control tables is already mapped) and, for any other custom exit/BAdI/enhancement/standard-mod, the `sap_custom_extensions` skill — do NOT re-trace what they document.**
