* ==== CLASS POOL YCL_FM_FI_COMPARE_BL ====
CLASS-POOL .
*"* class pool for class YCL_FM_FI_COMPARE_BL

*"* local type definitions
INCLUDE YCL_FM_FI_COMPARE_BL==========CCDEF.

*"* class YCL_FM_FI_COMPARE_BL definition
*"* public declarations
  INCLUDE YCL_FM_FI_COMPARE_BL==========CU.
*"* protected declarations
  INCLUDE YCL_FM_FI_COMPARE_BL==========CO.
*"* private declarations
  INCLUDE YCL_FM_FI_COMPARE_BL==========CI.
ENDCLASS. "YCL_FM_FI_COMPARE_BL definition

*"* macro definitions
INCLUDE YCL_FM_FI_COMPARE_BL==========CCMAC.
*"* local class implementation
INCLUDE YCL_FM_FI_COMPARE_BL==========CCIMP.

CLASS YCL_FM_FI_COMPARE_BL IMPLEMENTATION.
*"* method's implementations
  INCLUDE METHODS.
ENDCLASS. "YCL_FM_FI_COMPARE_BL implementation


* ---- YCL_FM_FI_COMPARE_BL==========CI ----
PRIVATE SECTION.
...
*      zzamountbrdiff TYPE ze_amountbr_diff,
  ...
  METHODS GET_EXCHANGE_RATE
    IMPORTING
      !IV_DATE TYPE DATUM
      !IV_TR_WAERS TYPE WAERS
    RETURNING
      VALUE(RV_KURSF) TYPE KURSF .
    ...
    LOOP AT LT_FMI INTO DATA(LS_FMI).
      ...
      IF LS_FM_TOTAL-BETRG <> LS_FI_TOTAL-BETRG.
        ...
        IF MO_BR_EXCHANGE_RATE_BL->CHECK_CONDITIONS( IV_BUKRS = LS_FMI-BUKRS
          ...
          ENDIF.
          "Get exchange rate and check if different from those in BKPF
          LV_KURSF = ME->GET_EXCHANGE_RATE( IV_DATE = LV_DATE IV_TR_WAERS = LS_FMI-TWAER ).
          IF LV_KURSF IS NOT INITIAL AND LV_KURSF <> LS_BSEG-KURSF.
            LV_KURSF = LS_BSEG-KURSF.
          ELSE.
            CLEAR LV_KURSF.
          ENDIF.
          "Calculate BR impact
...
*    "DELETE mt_bseg WHERE waers NOT IN mr_waers AND pswsl NOT IN mr_waers.
    ...
    SELECT S~BUKRS, S~BELNR, S~GJAHR, S~BUZEI, S~SHKZG, S~HKONT, S~DMBTR, S~WRBTR, S~PSWSL, S~BUDAT, K~WWERT, K~KURSF
           FROM BSIS AS S
           INNER JOIN BKPF AS K ON  K~BUKRS = S~BUKRS
                                AND K~BELNR = S~BELNR
                                AND K~GJAHR = S~GJAHR
...
*       CLIENT           = SY-MANDT
        DATE             = IV_DATE
        FOREIGN_CURRENCY = IV_TR_WAERS
        LOCAL_CURRENCY   = MV_WAERS
*       TYPE_OF_RATE     = 'M'
*       EXACT_DATE       = ' '
      IMPORTING
        EXCHANGE_RATE    = RV_KURSF
*       FOREIGN_FACTOR   =
*       LOCAL_FACTOR     =
*       VALID_FROM_DATE  =
*       DERIVED_RATE_TYPE       =
...
* ---- YCL_FM_FI_COMPARE_BL==========CO ----
PROTECTED SECTION.

* ---- YCL_FM_FI_COMPARE_BL==========CU ----
CLASS YCL_FM_FI_COMPARE_BL DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
