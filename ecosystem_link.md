...
| **AS-RUN operating model** — measured from P01 audit log (RSAU 15.6M, 4mo, host/dest/user): UNESCO **operates SAP from OUTSIDE — 80.6% external** (read-dominant system-of-record). Satellites named: **MuleSoft** (synctrigger fleet, 17 flows, bidirectional PPM↔SAP project+fund sync), **BRIDGE-RFC=ORION EAI** (procurement/travel portal, 92% read), named-user portal (write channel), WF-BATCH (HR), PBC engine. | `unesco-sap-brain` | 🚀 **READY (2026-06-21)** — **RE-EVALUATE any redesign hypothesis built on a dialog-driven AS-IS.** Handoff note: `Zagentexecution/sap_data_extraction/pending_from_sap_brain/HANDOFF_operating_model_2026-06-21.md` · evidence: `knowledge/system_operating_model_rfc.md` + `process_mining_table_analysis.md §7a-§7f` + engines in `process_mining/`. Golden DB `rsau_audit_history` (15.6M) for recompute. |
