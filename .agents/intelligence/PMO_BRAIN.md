...
> - **H101 [🔴 EGIPTO — DUE 2026-09-05, no tecnico]** — **Partir `D01K9B0FXF` en DOS transportes antes de liberar** y quitar de el la clave `350ID USTRD O01` de Indonesia. `YTFI_PPC_STRUC` es el interruptor: en cuanto entre en P01, toda factura de UNES a proveedor con banco egipcio queda bloqueada ese segundo (716 proveedores en el maestro). Secuencia: T1 (`T015L` + `YTFI_PPC_TAG`, inerte) ya; T2 (`YTFI_PPC_STRUC`) 1-2 dias antes del 5-sep. Rollback = borrar las filas de STRUC.
> - **H102 [🔴 EGIPTO — DUE 2026-09-05, negocio]** — **Avisar a la oficina de El Cairo y a BFM** de que el bloqueo empieza el dia del transporte, y **confirmar los cinco motivos** (`EG0 SUPP` / `EG1 SCVE` / `EG2 BEXP` / `EG3 CHAR` / `EG4 OTHR`) con BFM y CitiService Egypt. Es el unico item genuinamente abierto: lo tecnico esta probado. Owner: Mathewos Mehari / Treasury.
