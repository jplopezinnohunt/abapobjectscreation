...
> - **H71 [🔴 CONTROL — escalate to controls/treasury]** — **Write-channel SoD remediation.** `E_SILVA`/`L_NEVES` do GR (`BAPI_GOODSMVT_CREATE`) + invoice (`BAPI_INCOMINGINVOICE_CREATE1`) = self-approve the 3-way-match; `MP_ANCUTA`/`S_STANTIC` do PR change (`BAPI_PR_CHANGE`) + vendor-master change (`ZBAPI_VENDOR_CHANGE`) = direct spend; `UBO-RFC` posts FI. **Extends BCM SoD (claim #213) to the portal-as-user WRITE channel — where real control must look.** Action: confirm via AGR_* (H76), design control (split duty / add approval). Evidence: `knowledge/system_operating_model_rfc.md` §items-5-9.
...
> - **H74 [🟡 OPS — #1 failure mode]** — **Satellite TCP connectivity.** 272 syslog 10054-resets + 30 HTTP timeouts = the system's #1 failure mode (mirrors the 80%-external model: it fails at the CONNECTIONS, not the code). Deepen: correlate 10054 by satellite/host/hour → pinpoint the dropping connection (firewall idle-timeout? endpoint cycling?). Stand up TCP-stability monitoring to MuleSoft/ORION/SQL Server.
...
> - **H76** — **AGR_*/USR* pull from P01** → declared-vs-actual SoD ("right unused" / "done without the role"). Unblocks H71/H73. (Not in Gold DB today.)
...
> - **H78** — **operationalize the PROBLEMS accumulator**: wire `accumulate_problems.py` into the weekly schedule (SM21 ~7d retention); parse syslog `CENTDATA` (user/tcode/program/msg embedded) for richer signal.
