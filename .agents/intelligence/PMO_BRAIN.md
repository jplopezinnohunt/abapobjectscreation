...
| H18 | **Read YCL_IDFI_CGI_DMEE_AE/BH source — confirm XML PurpCd value** | #026 | Code | **Partially investigated #038 — hypothesis falsified, path wrong.** `YCL_IDFI_CGI_DMEE_AE` and `YCL_IDFI_CGI_DMEE_BH` **DO NOT EXIST in D01** (TADIR search returned 0). Closest matches: `YCL_IDFI_CGI_DMEE_FR`, `_FALLBACK`, `_UTIL`. All 3 extracted via RFC `READ REPORT` + CM001..CM050 method includes (7 method implementations total). **Zero `<Purp>/<Cd>` literals, zero SEPA PurposeCode enum values** found across all extracted ABAP. Conclusion: the PPC XML Cd value is NOT in these ABAP classes. Next path: inspect DMEE format tree (tx DMEE, stored as XML in DMEE_TREE_NODES), not ABAP classes. Downgraded from "read 2 classes" to "investigate DMEE tree". Output: `extracted_code/DMEE/`. |
