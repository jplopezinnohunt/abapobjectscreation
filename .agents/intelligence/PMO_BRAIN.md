...
> **Current count**: 0 Blocking | **9** High | **25** Backlog = **34 total** (Session #040: H30/H31/H32 added — Brain v2 Phases 1-3 were APPROVED in #039 architecture but never tracked as PMO items. G61/G62/G64 annotated as blocked-by/part-of the phase items. This is a PMO gap fix, not new scope.)
...
| **H30** | **Brain v2 Phase 1: Behavioral edges from code** | **#039** | **Brain** | **APPROVED architecture at `BRAIN_V2_ARCHITECTURE.md`.** Parse 896 ABAP files → ~5K behavioral edges (CALLS_FM, READS_TABLE, READS_FIELD, IMPLEMENTS_BADI, INHERITS_FROM). Create `brain_v2/` directory structure (graph.py, abap_parser.py, code_ingestor.py, cli.py). Expected: ~500 code nodes + ~3K READS edges + ~2K CALLS edges + ~50 BAdI edges. Validate with DMEE payment chain (FPAYP.XREF3 → YCL_IDFI_CGI_DMEE_FR → FI_CGI_DMEE_GET_PAYEE_DATA). G61-G64 are sub-tasks/follow-ons. **Spec:** Sections A.2 (30 node types), A.3 (45 edge types), B.1 (parser), E Phase 1, F (file layout) of `BRAIN_V2_ARCHITECTURE.md`. |
| **H31** | **Brain v2 Phase 2: Config + Integration + Transport edges** | **#039** | **Brain** | Blocked by H30. Ingest T042Z/T042A/BCM → config edges, CTS 108K objects → transport edges, RFCDES/TFDIR → integration edges. ~160K new edges. Spec: Sections B.2, B.4, E Phase 2. |
| **H32** | **Brain v2 Phase 3: Process overlay + Query engine + CLI** | **#039** | **Brain** | Blocked by H31. Process mining → graph overlay, 4 query types (impact/dependency/similarity/gap), CLI interface, HTML companion. Spec: Sections B.3, C.1-C.5, E Phase 3. |
...
| # | Task | First raised | Notes |
|---|------|-------------|-------|
| G61 | **Brain v2: fix impact query direction + v1 migration (73K fund nodes)** | #039 | **Blocked by H30.** Impact query searches forward from changed node, but READS_FIELD points code→field. Need backward search for "who reads this field". Also migrate v1 funds/transports/knowledge nodes. Spec: Section C.1 (impact direction model) + G (migration). |
| G62 | **Brain v2: ingest DMEE→FPAYP.XREF3 edge from h18_dmee_tree_nodes.csv** | #039 | **Part of H31 (Phase 2).** The H18 finding (PurposeCode chain) is documented but NOT in the brain as edges yet. Must parse DMEE_TREE_NODE CSV for USES_DMEE_TREE + CONFIGURES_FORMAT + READS_FIELD edges. Spec: Section B.2 config_ingestor. |
