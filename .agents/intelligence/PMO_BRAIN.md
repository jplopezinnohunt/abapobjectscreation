...
> **Prior**: Session #058 (2026-04-20) â€” **INC-000006906 closure + bank reconciliation family inventory.** 5 new H-items (H54-H58): transport YTBAE002 fix to P01, STAD trace YTR1/YTR2/YTR2_HR, extract YCL_FI_BANK_RECONCILIATION_BL, YTBAI001 SMARTLINK status, YTBAM002_HR_UBO TADIR-delete. Dormant MODE 'E' landmines preserved on YTBAE001 + YTBAE001_HR (user declined speculative fix). New claims 52/53/54. +~30 new objects (YTR3 family, SAPDBSDF, PUT_BSIS, SKB1, FB08, F-04, FBRA, YBANK_ACCOUNTS_FO_OTH). 3 new feedback rules (mode_e_bdc + d01_adt_fallback + time_out_stack_blames_ldb). Retro: `Zagentexecution/retros/session_058_retro.md`.
> **Prior**: Session #054 (2026-04-14) â€” **Formalization audit + Core Principles bootstrap.** User-driven audit revealed 13 of 16 session #053 commitments were never formalized in PMO. Response: 3 Core Principles established (CP-001 Knowledge over velocity, CP-002 Preserve first, CP-003 Precision evidence facts) as constitutional layer 0 of brain_state.json. 4 new operational rules (86 total: retro_to_pmo_bridge CRITICAL, never_drop_columns HIGH, sample_before_aggregating + explicit_aggregation_filter MEDIUM). H42 schema migration executed: claims.evidence_for/against str â†’ list[typed]. 11 new H-items (H41-H51). H36 refreshed (20â†’71 blind_spots). See `brain_v2/core_principles/core_principles.json` + `knowledge/session_retros/session_054_retro.md`.
> **Session #050** prior: Brain v3 validation + incident methodology bootstrap. 12 deliverables. H35 closed. 7 new feedback rules (58 total). 2 new brain layers (incidents=11, blind_spots=12). Brain coverage metric introduced: 75.6%. Object count 102â†’136 via force-include. New skill `sap_incident_analyst` + subagent `incident-analyst`. New session-close Phase 4b (Capture SAP Learnings).
...
INCOMING (2026-08-19 â€” s101: Egipto pasa de especificacion a fichero probado, y la mecanizacion caza un defecto vivo).**
> Done-since: `INC-EGYPT-PPC` construido en D01 (ticket real **INC-000016101**) y **probado por las dos mitades** â€” bloquea sin codigo (`ZFI-036`, GB931 UNES paso 012) y renderiza con codigo (fichero `pain.001` con `<Ustrd>/Payment for goods or services received/INV/224938`, identico a la prediccion hecha simulando CM003/CM004 antes de generarlo). Cero ABAP confirmado por ejecucion. Cazado y reparado de paso un dano colateral a **Indonesia** dentro del transporte de Egipto. Claims 524-529, 4 reglas nuevas, 2 checks nuevos, los dos companions actualizados. Artefacto: `Zagentexecution/tasks/2026_08_19_egypt_ppc_d01_test/`.
> - **~~H101~~ ~~H102~~ [CERRADOS 2026-08-20 â€” Egipto NO hacia falta]** â€” SG confirmo que no existe codigo nuevo para Egipto y que el canal Citi no lleva ese flujo (**SocGen 93,1% del corredor vs Citi Egipto 0,9%, y en cheque prenumerado, que no es RTGS ni CBFT**). No se transporta nada. Claim 530. **Sustituidos por H106.**
> - **H112 [ðŸ”´ DURABILIDAD â€” lo unico con riesgo de PERDIDA, no de retraso]** â€” El disco de copias `D:\claude_backups` **esta desconectado**; ultima copia 2026-08-19 17:56. Mientras siga asi, el **Golden DB (15,2 GB, gitignored) existe solo en este disco**. Y hay trabajo que solo vive aqui: **sin remoto y sin commitear** en `FINCLOSSING` (15), `SCRP` (4), `unescore20-PPM-brain` (7); **sin subir a origin** en `unesco-sap-brain` (4) y `unescrp` (1). Este proyecto esta limpio y en origin. El medidor lo ve solo desde s102 (`meta_capability.assets_backed_up` paso de estimacion ciega a MEDIDO, y hoy marca 0.00 con la razon correcta: *sin destino accesible*). **Que falta:** reconectar el disco y correr `python scripts/backup_golden.py --dest D:\claude_backups`; y decidir si los tres proyectos sin remoto lo tendran, porque hoy no hay nada que los proteja.
> - **H113 [ðŸŸ  REUBICACION â€” quedan 50, ya no hay ninguna CRITICAL]** â€” Las **22 CRITICAL sin punto de uso se reubicaron a 0** en s102 (a checks, scripts, skills, docs de dominio y `CLAUDE.md`). Quedan **50 reglas sin cita**, todas HIGH o menores. **Que falta:** el mismo tratamiento â€” por cada una, contestar *donde esta alguien en el momento en que aplica* y citarla ahi. **No se retiran** (regla `feedback_never_retire_anything_without_evidence`, CRITICAL): 'no citada' es un proxy, no evidencia. Mecanizacion del corpus: 11% -> 16% (37 de 221). El indicador a subir es ese, no el numero de reglas a bajar.
> - **H114 [ðŸŸ¡ HUERFANOS â€” 8, destapados y no arreglados]** â€” `artifact_wiring_check.py` (nuevo en s102) encuentra **8 artefactos que nadie invoca**: 6 scripts sin llamador (`agent_trace_hook.py`, `backfill_closing_activities.py`, `backup_projects.py`, `brain_coverage_audit.py`, `build_closing_activities_pdf.py`, `ingest_fx_revaluation_structured.py`) y 2 JSON que ni su autor relee. La mayoria son anteriores a s102. **Que falta:** por cada uno, encadenarlo o dar la evidencia de que ya no aplica â€” nunca borrarlo por no estar citado.
> - **H115 [ðŸŸ¢ REUBICAR, NO MECANIZAR â€” dos reglas mal colocadas]** â€” `feedback_predict_the_output_before_you_generate_it` y `feedback_a_warning_in_a_document_is_not_a_measurement` son **epistemologia, no procedimiento**: describen como razonar, no un momento con una accion. Su sitio es **CP-003** como elaboracion, no las posiciones 219/220 de un fichero que nadie abre. **Que falta:** decidir si los Core Principles admiten elaboraciones â€” hoy son tres parrafos â€” y si no, aceptar que son prosa y dejar de contarlas como deuda de mecanizacion.
> - **H106 [ðŸŸ  HIGIENE â€” desarmar D01 â€” JP CONFIRMA 2026-08-21]** â€” Decidido borrar la configuracion COMPLETA de Egipto en D01 (no solo el interruptor) y el transporte. PRE-readback guardado en `Zagentexecution/tasks/2026_08_19_egypt_ppc_d01_test/PRE_DELETE_snapshot_D01.json`: 5 filas T015L + 10 YTFI_PPC_STRUC + 1 YTFI_PPC_TAG + las 17 claves del transporte + las 18 filas de Indonesia como control. Orden: `YTFI_PPC_STRUC` primero (apaga el bloqueo), luego las inertes, luego `SE10`. **Riesgo a vigilar:** borrar en SM30 puede capturar la clave de un pais vecino, y ahora la operacion es un BORRADO. ~~Verificar despues con `config_transport_prerelease_check.py` y el diff completo de las tres tablas.~~ **RETIRADO 2026-08-26** -- motivo: el instrumento no puede ser la puerta de un BORRADO sobre customizing (A40 `DEFECTO_VIVO`). **EN SU LUGAR:** verificar despues con el **diff completo de las tres tablas D01 vs P01** (el metodo manual que encontro Indonesia el 2026-08-19), contra el PRE-readback `PRE_DELETE_snapshot_D01.json`, incluidas las 18 filas de Indonesia como control. **NO USAR `config_transport_prerelease_check.py` como puerta de este borrado**: sobre `D01K9B0FXE` devuelve exit 0 sin analizar nada (medido 2026-08-25 -- imprime 'E071K vacio' porque las claves viven en E071K de la TAREA, no de la orden), y una clave BORRADA (inexistente en ambos sistemas) sale `[NO-OP]` con exit 0, indistinguible de un troceo fallido (`brain_v2/methods/algorithms.json:1301` modo C). Su exit 0 aqui NO SIGNIFICA NADA. â€” El interruptor sigue ARMADO en D01: cualquier `FB60` a uno de los 716 proveedores con banco egipcio se bloquea sin motivo, y el transporte `D01K9B0FXE`/`D01K9B0FXF` existe sin liberar â€” si alguien lo libera por error, el bloqueo arranca en P01. **Accion:** borrar las filas `EG` de `YTFI_PPC_STRUC` (es el interruptor, rollback = un delete) y borrar el transporte. `T015L` y `YTFI_PPC_TAG` pueden quedarse, son inertes. No se pierde nada: la configuracion exacta esta en el companion Â§11, en los claims 526/527 y en `Zagentexecution/tasks/2026_08_19_egypt_ppc_d01_test/`. Se preserva el CONOCIMIENTO, no el artefacto.
> - **H111 [ðŸŸ¡ DEFECTO PRE-EXISTENTE, menor]** â€” `companions/treasury_operations_companion_v2.html` tiene **3 `<div>` sin cerrar** (235 abren / 238 cierran). Verificado que es previo a los cambios de s102: la diferencia neta que introdujo esta sesion es 0. No se toco a ciegas. **Que falta:** localizar los tres y cerrarlos, y de paso mirar si el resto de companions estaticos tienen el mismo problema â€” hoy nadie lo comprueba, y un HTML mal balanceado renderiza distinto segun el navegador.
> - **H110 [ðŸŸ  EXTRACCION â€” sin esto el modelo de bancos esta cojo]** â€” **`FEBKO` es un extracto PARCIAL**: 31.416 filas, solo 38 bancos y **solo 3 sociedades** (UNES, ICBA, MGIE). **`SOG01`, con 1,9M de pagos, tiene CERO filas**; faltan UBO, ICTP, IIEP, UIL y UIS enteras. Sin ello no se puede responder que bancos reciben extracto electronico, y cruzarlo produce conclusiones invertidas â€” casi se publica que los hubs no reciben extractos. Claim 535. **Que falta:** extraer `FEBKO` completo (todas las sociedades) por `gold_refresh.py`, y entonces anadir al modelo de bancos la dimension **recibe extracto / es cuenta receptora** â€” hoy declarada MISSING_INPUT y no como columna de ceros. **Ya se ve algo aun con el extracto parcial:** `SCB04`, `CIT13`, `SCB18`, `SBN01` y `SCB19` tienen extracto y CERO pagos â€” son **cuentas RECEPTORAS**, un rol al que un modelo derivado solo de `REGUH` es ciego por construccion, porque `REGUH` solo ve dinero saliendo.
> - **H109 [ðŸŸ¢ OPORTUNIDAD MEDIDA â€” H_IMPROVE, y la palanca NO es la que parece]** â€” **36 de 37 bancos de oficina de campo pagan el 100% en cheque.** Brasil (7%) y Canada (0%) son las excepciones que prueban que se puede salir del papel. Pero de las ~10.400 lineas en papel local, **solo 647 (6%) son candidatas claras a centralizar** â€” Libano 288, Jordania 206, Etiopia 153, y en las tres el hub YA paga a ese pais. **5.766 lineas (55%) estan bloqueadas por DATOS BANCARIOS DE PROVEEDOR ausentes**, y 3.952 por jurisdiccion. Claim 534. **La convergencia que esto revela:** los 1.225 proveedores egipcios sin LFBK (claim 489), los 8.149 sin CtrySubDvsn de INC-PSTLADR-NOV2026 y estas 5.766 lineas **son la misma causa** â€” el maestro de proveedores incompleto. Tres frentes, una palanca. **Que falta:** (a) confirmar con Tesoreria la clasificacion de jurisdiccion, que hoy es conocimiento mio y no un dato del sistema; (b) rehacer el corte por IMPORTE y no por lineas; (c) decidir si la campana de datos bancarios de proveedor se ataca como UN proyecto en vez de tres.
> - **H108 [ðŸŸ¡ HIGIENE DE MAESTRO â€” bancos casa muertos]** â€” Tres cuentas de banco casa configuradas en UNES sin mover dinero desde hace anos: **CHA01** (JPMorgan Chase, ultima ejecucion **2020-01**), **DNB01** (Noruega, **2017-07**) y **WEL01** (Wells Fargo, **2023-01**, 4 lineas ese ano). Las tres son cuentas de TESORERIA entre cuentas propias (metodo A). **SCB14** (Standard Chartered) si esta vivo (2026-04). Ademas **DEU01** â€” hub de UIL con 109 destinos â€” no ejecuta un pago desde **2023-09**: no es una cuenta de tesoreria, es un hub que se quedo callado, y merece pregunta aparte. Claim 533. **Que falta:** confirmar con Tesoreria si se cierran o se mantienen abiertas a proposito. Una cuenta abierta y sin uso es superficie de riesgo, no un dato neutro.
> - **H107 [ðŸŸ¢ MODELO â€” hecho, queda extenderlo]** â€” **`brain_v2/house_bank_roles.py`** contesta ahora "quien sirve este corredor y con que rol" en un comando (paso 2d del rebuild, artefacto `house_bank_roles.json`). Nacio de que el dato de Egipto estaba medido desde el 17-ago y disperso en prosa. **Que falta:** llevarlo al capability_model como evidencia de `Payment_BCM / F_INTERFACE_FILE` y `A_PROCESS`, y decidir si el mismo patron â€” rol operativo derivado en vez de declarado â€” aplica a otros canales (RFC destinations, interfaces de fichero).
> - **H103 [ðŸŸ  DEFECTO VIVO EN P01]** â€” **`T015L INA` tiene dos espacios** entre codigo y narrativa: `'P1203  Maintenance of international institutions such as offices of IM'`. India separa con `;`, asi que emite `P1203; Maintenance of...;INV;<XBLNR>` con un blanco de mas hacia el banco. Unica fila rota de las 73. Arreglo = borrar un espacio por SM30, cero ABAP. **No medido**: cuantos pagos han usado `INA` (REGUP no esta en el Gold DB). Claim 529 â€” encontrado por el check E/F en su primera corrida, nadie lo buscaba.
> - **H104 [ðŸŸ  INSTRUMENTAL â€” el guardian tiene el defecto que vigila]** â€” **`knowledge_reachability_check.py` sale con exit 0 mientras el indice que vigila miente.** Comprueba que el `id` este en `BRAIN_INDEX.md`; nunca comprueba que lo que el indice DICE sea actual. Toda la sesion s101 el indice decia `SPEC_READY` y proponia un plan equivocado en tres puntos, y el check lo daba por bueno. Es el patron del claim 496 â€” *el control prueba presencia, no correccion* â€” reproducido en nuestro propio instrumental. **Que falta:** que el check compare el `status`/`next_action` del indice contra el store, no solo la presencia del id.
> **â®• FRONTERA DE ALINEAMIENTO DE CUSTOMIZING (JP 2026-08-20, s102).** Alinear master data NO alinea el sistema: las cuentas GL de UNES quedaron al 100% en los tres sistemas y el customizing sigue derivado. **TRES frentes, y de cada uno falta DECIDIR EL METODO** â€” encontrar el canal o autorizar una excepcion de escritura directa (registro EXC-* en `.agents/skills/sap_master_data_sync/SKILL.md`). Escalera de canales: (1) API estandar Â· (2) BC-Set, que escribe por el framework y graba en orden pero exige crear el BC-Set a mano una vez por sistema Â· (3) escritura directa, prohibida salvo excepcion nominal con las diez puertas.
> - **H114 [ðŸ”´ ALINEAR OB09 / `T030H` â€” y hay un defecto vivo de 20 M EUR]** â€” **(1) DEFECTO REAL, no de alineamiento:** `4041011` tiene **4 partidas abiertas de 5.000.000 EUR** (20-05, 20-07 y **dos del 20-08-2026**) y **no la selecciona ninguna variante de F.05**, **una de ellas al HABER**, asi que la exposicion NETA son **10 M EUR** (11.569.086,65 USD) y **no se revaluan**. Esta en la posicion de balance `FS10 1.1.2.1 Short Term Deposits` -dentro del grupo de inversiones- con `XOPVW='X'`; y `4041013`, que SI esta en la variante, tiene la misma naturaleza y solo UNA partida EUR abierta. Medido en vivo con `BSIS` cuenta a cuenta. `4041012` (0 abiertas) y `4041014` (solo USD) son **inertes**, no defectos. **Preguntar a Tesoreria si 4041011 debe entrar en `UNES_DEPOSIT`.** **(2) ALINEAMIENTO:** `T030H` diverge entre sistemas â€” el check detecta ademas dos `[DERIVA]` (`0001122421`, `0001122424`) con fila en P01 y no en D01. **Metodo:** `T030H` **si es transportable** (probado con `D01K9B0FXP`: funcion Q = **tarea**, 2 claves en E071K; ~~0 intrusas~~ **RETIRADO 2026-08-26**: era **vacuo**, no medido -- en `T030H` el check deriva el eje de entidad como el 1er campo clave tras `MANDT` (= `KTOPL`) y en D01 solo existe `UNES`, asi que la rama INTRUSA no podia disparar jamas; el discriminador real, `HKONT`, no se mira. El alcance de ese transporte **NO ESTA VERIFICADO**, y la **ORDEN padre** (`E070 WHERE STRKORR='D01K9B0FXP'`), que es lo que se libera, no la ha mirado nadie), asi que el canal ortodoxo existe: configurar en D01 y transportar. **No necesita excepcion** -- pero **ATENCION**: esa conclusion se apoya en la **EXISTENCIA del canal**, que sigue en pie, **no** en la limpieza del alcance. Medidor: `ob09_vs_variant_check.py` (exige las TRES condiciones desde s102). Claims 549, 540-refutado.
> - **H113 [ðŸŸ¢ ALINEAR VARIANTES DE F.05 â€” CANAL RESUELTO 2026-08-21, queda ejecutar]** â€” **`RS_CREATE_VARIANT_RFC` escribe y `RS_VARIANT_DELETE_RFC` borra**, probado extremo a extremo en D01 con una variante desechable y verificado releyendo (claim 550). **No hace falta excepcion**: es peldaÃ±o 1, API estandar. `RS_VARIANT_CHANGE_RFC` NO vale (sin tabla de contenido, es de dialogo), asi que modificar una existente = **borrar y recrear** con el contenido completo â€” destructivo, exige snapshot PRE y readback POST. **Y el aviso que cuesta caro: lo que no se envia SAP lo rellena con defectos EN SILENCIO** (la prueba perdio 9 parametros de pantalla), asi que hay que mandar TODOS los parametros, no solo la seleccion de cuentas. Estado del problema: â€” `UNES_DEPOSIT` tiene **tres contenidos distintos** con el mismo nombre: P01 **17 valores sueltos EQ** (ya con 4041018), D01 rango `4041011-13` + 2 sueltas redundantes, V01 rangos `4041011-14` y `5091010-12` + 10 sueltas. **`VARID.TRANSPORT='F'`: no se transportan NUNCA.** Consecuencia: **D01 y V01 no reproducen F.05 de produccion** â€” 4041017, la unica cuenta con 198 partidas EUR abiertas y 12 documentos `FBB1`, **no esta seleccionada en ninguno de los dos**. **Metodo pendiente de decidir:** no hay API de escritura de variantes (`RS_VARIANT_CONTENTS_RFC` solo LEE); las opciones son teclear a mano en cada sistema, o investigar `RS_CREATE_VARIANT_RFC` / `RS_VARIANT_CHANGE_RFC`, que **si estan remote-enabled** y no se han probado. Se lee con el agente `variant-intelligence`. Claim 542.
> - **H112 [ðŸŸ  ALINEACION DE CUSTOMIZING â€” la FSV, y esta BLOQUEADA por un candado de cliente]** â€” **Alinear master data NO alinea el sistema.** Con las cuentas GL de UNES al 100% en los tres sistemas (s102: 2 creadas en D01, 33 en V01, readback OK), la **version de balance** sigue derivada. ~~`FAGL_011ZC` faltan **195** intervalos en D01 y **131** en V01; `FAGL_011PC` **287** y **278**; `FAGL_011QT` 179 y 96; `FAGL_011SC` 51/22 mas **229/213 que DIFIEREN**.~~ **REETIQUETADO COMO HISTORICO 2026-08-26** -- motivo: el `287`/`278` de `FAGL_011PC` se midio por **clave subrogada** y no cuenta nodos ausentes; y el estado ya no es el de entonces. **Medido 2026-08-20 (s102), estado de ENTONCES:** `FAGL_011ZC` faltan **195** D01 / **131** V01; `FAGL_011QT` 179/96; `FAGL_011SC` 51/22 mas **229/213 que difieren**; `FAGL_011PC`: **numero de nodos ausentes DESCONOCIDO** (medido por clave subrogada). **ESTADO ACTUAL EN CONFLICTO Y SIN RESOLVER:** `brain_v2/methods/algorithms.json` (2026-08-25) dice **0 faltan / 0 difieren / 0 extra** en las 8 tablas contra D01 y V01; `.agents/skills/sap_master_data_sync/SKILL.md:153` dice **V01 parcial, falta `FAGL_011QT`**. **Re-medir antes de dar H112 por cerrado.** `T011`/`T011T` identicas: el marco coincide y el contenido no. **El bloqueo:** SCU0/SCMP devuelven **`TB569`** â€” `T000.CCCOPYLOCK='L'` en P01 (nivel 2: sin disponibilidad externa). Es un candado DISTINTO del de modificabilidad y hay que leer los tres (`CCCORACTIV`, `CCNOCLIIND`, `CCCOPYLOCK`). **Dos salidas:** (A) Basis baja el candado a **`X`** en SCC4 â€”nivel 1, sigue protegiendo contra sobrescritura y **si** permite la lectura de comparacionâ€”, ventana y vuelta atras; (B) sin SCU0: `RFC_READ_TABLE` **no** esta afectado, `fsv_alignment_check.py` ya lee P01 y emite la especificacion, y se teclea en **D01 por `OB58`** grabando en orden, con transporte **D01â†’V01**. **No hay API RFC para la FSV** (comprobado en `TFDIR`) y `FAGL_011*` son estandar, asi que el INSERT plano esta prohibido. **Decision previa que ahorra la mitad:** la deriva se parte casi por igual entre **FS10** (111 D01 / 65 V01) y **FS11** (84 / 66) â€” determinar si FS11 sigue viva antes de tocar nada. **RUTA AUTORIZADA (JP 2026-08-20): EXC-001** en `.agents/skills/sap_master_data_sync/SKILL.md` -> 'REGISTRO DE EXCEPCIONES AUTORIZADAS'. Unica excepcion vigente para escritura directa: FAGL_011PC/QT/ZC/SC, chart UNES, destinos D01 y V01, SOLO INSERT de filas ausentes, con las DIEZ PUERTAS y la G10 obligatoria (registrar las claves en una orden de customizing ~~y validar con `config_transport_prerelease_check.py`~~ -- **RETIRADO 2026-08-26**). **ATENCION: G10 esta hoy SIN VALIDADOR.** `config_transport_prerelease_check.py` esta en `DEFECTO_VIVO`: no analiza la ORDEN que se libera (las claves viven en E071K de la TAREA) y no lee `E071`, que es justo donde deja rastro el `R3TR TABU` que G10 prescribe. Registrar las claves igualmente y **dejar la validacion de alcance marcada como PENDIENTE**, verificada a mano por diff completo de las tablas; **no dar G10 por pasada con un exit 0**. Alternativa ortodoxa si se prefiere: `OB58` en D01 grabando en orden + transporte D01->V01. Hoja de lanzamiento: `knowledge/domains/FI/fsv_alignment_runsheet.md`. Medidores: `fsv_alignment_check.py` y `ob09_vs_variant_check.py` -- **ANTES de creer su exit 0, ASEVERAR EL SID**: ambos aceptan `--systems` sin validarlo, y un SID sin bloque `SAP_<SID>_` en el `.env` cae al bloque generico, **que ES D01 y lleva password**, asi que conectan con EXITO al sistema equivocado y lo certifican alineado (claim 597; A43 `DEFECTO_VIVO`). `ob09_vs_variant_check.py` arrastra ademas A47 (`DEFECTO_VIVO`, claim 599). Claims 546, 547, 548.
> - **H113 [ðŸŸ  LAS VARIANTES DE F.05 DIVERGEN Y NO SE TRANSPORTAN]** â€” `UNES_DEPOSIT` tiene **tres contenidos distintos** con el mismo nombre: P01 **16 valores sueltos EQ**, D01 rango `4041011-13` + 2 sueltas redundantes, V01 rangos `4041011-14` y `5091010-12` + 10 sueltas. `VARID.TRANSPORT='F'`: **no se transportan nunca**, cada sistema tiene la suya editada a mano. Consecuencia dura: **D01 y V01 no reproducen F.05 de P01** â€” la cuenta que de verdad se valora en produccion (`4041017`, 402,7 M EUR) **no esta seleccionada en ninguno de los dos**. Y el cruce OB09 x variante da cuentas ACTIVAS configuradas que ninguna variante selecciona: **P01 4041011, 4041012, 4041014**; D01 4041014 y 4041017; V01 4041017. **MEDIDO 2026-08-21 (claim 551):** la divergencia es HETEROGENEA -- D01 118 diferencias (76 seleccion + 20 modo + 22 residuo) en 11 variantes; V01 solo 15 (11 seleccion + 4 residuo) en 3. **PLAN POR FASES, nada ejecutado:** (1) V01 entero, bajo riesgo; (2) D01 solo seleccion limpia -- IBE_OI, ICTP_OI, IIEP_OI, UNES_DEPOSIT, UNES_OI_AR/AP; (3) **DECISION HUMANA** para ICBA_*, MGIE_*, UIS_OI y UIL_OI, porque alinearlas BORRARIA el nombre de sesion batch (PAR_BNAM, vacio en P01 y con valor en D01) y cambiaria la bandera de alcance (PA_WEREF en P01 frente a PA_WEREN en D01); UIL_OI ademas difiere en 36 entradas de seleccion. Herramienta: `Zagentexecution/tasks/2026_08_21_variant_alignment/variant_align.py` (dry-run por defecto, snapshot PRE, restauracion automatica, verificacion POST). **Que falta:** (a) preguntar a Tesoreria si 4041011/4041012 â€”con exposicion EUR realâ€” se dejaron fuera a proposito; (b) decidir si se alinean las variantes de D01/V01 a mano. Claims 540/542. Se lee con `RS_VARIANT_CONTENTS_RFC`, ver agente `variant-intelligence`.
> - **H105 [ðŸŸ¡ COBERTURA]** â€” **`REGUP` no esta en el Gold DB**, y eso deja dos cosas sin medir: el check D de `ppc_country_consistency_check.py` (uso degenerado â€” si un codigo se come el 60% de los pagos, el control es cosmetico) y el impacto real de H103. Es una extraccion, no un analisis.

> **â®• INCOMING (2026-08-18 â€” s099: los quality_checks empiezan a correr solos, y encuentran cosas).**
> - **H91 [ðŸ”´ DATA QUALITY â€” pendiente, sin dueÃ±o asignado]** â€” **191 proveedores con `AKONT` atÃ­pica**, misma clase de defecto que INC-000006073. Detectado por `Zagentexecution/quality_checks/vendor_master_integrity_check.py` en su primera corrida automatizada. 127 grupos `(KTOKK,BUKRS)` analizados; el patrÃ³n dominante es `GVNT/UNES` usando `0002021011` cuando el 97% del grupo (2.378 pares) usa `0002021032` â€” 2,82% de cuota, muy por debajo del umbral del 5%. Ninguno tiene `GGB1`, asÃ­ que no hay sustituciÃ³n que lo explique. Incluye entidades reales (IUCN, CNRS, delegaciones permanentes, comisiones nacionales). **QuÃ© falta:** decidir si es error de maestro o una categorÃ­a legÃ­tima mal etiquetada; si es error, corregir `LFB1-AKONT` por el canal autorizado (nunca escritura directa a P01). El check ya estÃ¡ en el tier gate, asÃ­ que la cifra se vigila sola a partir de ahora.
> - **H92 [ðŸŸ  MODELO]** â€” **40 de 43 algoritmos no declaran `lands_in`** (`brain_v2/methods/algorithms.json`). El campo se aÃ±adiÃ³ en s099 para la regla de "descubrir sin aterrizar" y sÃ³lo se aplicÃ³ a 3. Un algoritmo sin `lands_in` puede descubrir y no tener dÃ³nde dejarlo: es la versiÃ³n sistÃ©mica del hueco que JP seÃ±alÃ³ con Purpose of Payment.
> - **H93 [ðŸŸ  RECUPERABILIDAD]** â€” **los companions no eran buscables por tema.** El trabajo de DMEE vive en `BCM_StructuredAddressChange.html` (965 menciones) titulado "BCM Structured Address Change" y etiquetado "finance": nada en su nombre ni en el registro dice DMEE. Resuelto para la bÃºsqueda (`graph_queries.py search` ya alcanza los 15 companions con DMEE), **pendiente**: los companions siguen sin aparecer en `BRAIN_INDEX.md`, asÃ­ que el conocimiento profundo no se anuncia solo al arrancar una sesiÃ³n.
...
> **â®• INCOMING (2026-06-21 â€” log accumulator + RSAU audit + process-mining Tier 0/1/2; facts promoted by steward S-083, claims #212â€“216).** Pending backlog (H66â€“H70):
> - **H66 [BLOCKING for schedule]** â€” BASIS **keytab/headless SNC** so the 14-day `accumulate_logs.py` runs unattended (interactive Kerberos ticket dies ~10h, fails overnight â€” claim #215). JP raises the ticket.
> - **H67** â€” complete volatile-log capture: **SM21 syslog** `SALC_MSC_READ_SYSLOG` returned 0 lines (verify ONLY_LOCAL/window/per-server); **ST22 dumps (SNAP)** need the dump FM (not RFC_READ_TABLE); rerun **9 failed RSAU chunks** (idempotent); decide+apply the **RSAU type-retention filter** (keep Dialog Logon+Transaction Start+User Master Changes+High; drop RFC machine noise ~80%).
> - **H68** â€” process-mining next tiers: **formal alignments conformance + temporal bottlenecks**; **label custom cockpits** YFM1/ZICTP_COCKPIT/YKBLP_N via the brain (comprehension gap = real master/budget maintenance, the moat); **SoD permission-level (AGR_*)** + declared-vs-actual reconciliation; **roll methodology** to Payment/FI/Treasury/PSM; LLM-native labeling. Scripts in `process_mining/` (committed 2a2b94f/c4926f0/9b4fb1a/62b5a9e). Maturity 12.5%â†’~25%.
> - **H69** â€” Gold DB catalog: extend beyond ~26/306 tables; **rename upgrade tables to real SAP names** (`tpalog_upgrades`â†’`tpalog`, `tbtco_upgrade2026_jobs`â†’`tbtco` subset). See `knowledge/gold_db_table_catalog.md`.
> - **H70** â€” decisions: move `accumulate_logs.py` to git-tracked `scripts/` (currently gitignored local-only); Golden DB (~6.4GB) + `~/.claude` memory remain LOCAL-ONLY â†’ need disk/offsite backup.
>
> **â®• INCOMING (2026-06-21 cont. â€” OPERATING-MODEL discovery + PROBLEMS + SoD; steward S-085, claims #220â€“231).** Done-since: **H67 âœ…** (problems accumulator `process_mining/accumulate_problems.py` captures ST22 dumps + SM21 syslog in one P01-safe call `/USE/BL_GET_SHORTDUMPS`; RSAU signal view `rsau_audit_signal` = 36% signal; SM21 syslog retains only ~7d). **H68 partial** (cockpits labeled; AGR_* still open). New actionables (H71â€“H80), redefined from the full session so findings escalate as TASKS, not notes:
> - **H71 [ðŸ”´ CONTROL â€” âœ… ANALYZED 2026-06-22 (a+b+c); escalate remediation to controls/treasury]** â€” **Write-channel SoD.** CONFIRMED at behavioral AND declared (AGR role) levels â€” full writeup `knowledge/domains/Security/h71_write_channel_sod_remediation.md`, claims #237â€“240. **Conflict 1** = BrasÃ­lia AP (E_SILVA/L_NEVES/**+C_SOUZA/+B_LOPES**, identical `Y_UBO_*` bundle) post GR+invoice+PO (ACTVT 01/02; vendor-bank role is display-only) = self-approve 3-way-match â†’ **R$ 264.7M (~US$49M), 9,411 POs, 100% with GR**. **Conflict 2** = HQ ICTP (MP_ANCUTA `Y_ICTP_SIS` vendor+PR+PO change; S_STANTIC) control 5,084 vendors + 77K PR changes â†’ **~EUR 11.8M** cleared AP to controlled vendors. **ROOT:** `S_RFC=*` (Y_ICTP_COMMON, Y_UBO_ADMINISTRATION) + custom `ZBAPI_VENDOR_CHANGE` does NO `F_LFA1` check (S_STANTIC changed 6,972 vendors with no F_LFA1 grant) â†’ role-split alone is cosmetic; control must move to call/data layer + SAP-native approval (T055F dual-control vendor bank, MRBR invoice release, PR/PO release creatorâ‰ releaser). **Remaining:** execute remediation (BASIS/Security + Controls); pull MSEG.USNAM for same-PO self-match. Evidence: `knowledge/system_operating_model_rfc.md` Â§items-5-9.
>   - **âš ï¸ SPECIAL HANDLING (not an autonomous fix â€” human escalation required):**
...
>     2. **Custom-BAPI fix follows BROADCAST-007:** own Z-objects only â†’ D01 dev â†’ **released transport** â†’ ATC-REST gate â†’ **JP 4-eyes**. No ADT write; no change in P01.
>     3. **ðŸ”´ NO QAS** = the structural test risk for these role/config changes â€” flag it explicitly; there is no QA tier to validate before prod (escalate as the gating risk).
>     4. **Sensitive** â€” names real users with a self-dealing *capability* (capability â‰  proven fraud). Route through the controls channel; do NOT broadcast names widely.
>     5. **Ecosystem (ADR-007):** candidate **BROADCAST to `unesco-sap-brain`** â€” the S_RFC=* + integration-write-channel weakness is architecturally relevant to the S/4 redesign (the write-side control model). Decide with JP before sending.
> - **H72 [ðŸ”´ AUDIT]** â€” **P2P 3-way-match violations:** 70 IR-before-GR ($713,341, claim #214) + ~~32 invoice+payment SoD (T_ENG/I_MARQUAND, #213)~~ **RETIRADO 2026-08-26** -- motivo: el `32` se conto agrupando por `SLGTC`, el tcode **LANZADOR**, cuando el arrancado vive en `PARAM1` (coinciden en 8 de 1.235.225 filas); claim #213 **SUPERSEDIDO**. **En su lugar:** solapamiento SoD factura+pago en `T_ENG` e `I_MARQUAND` (corroborado por el doble control de BCM, claim #6, y por `AGR_1251`, claims #237-240); **el RECUENTO de usuarios en conflicto se retiro y se DESCONOCE** -- hay que re-derivarlo con `PARAM1`. + 17% PO-no-receipt (open commitments). Remediation + recurring detector. **ACCION PREVIA: re-correr `tier2_sod.py` arreglado ANTES de llevar H72 a ningun comite.** Las otras dos patas **SOBREVIVEN INTACTAS**: los 70 IR-before-GR ($713.341) y el 17% PO-sin-recepcion salen de `p2p_conformance.py` sobre `EKKO`/`EKBE` -- tablas de negocio, no el audit-log -- y no pasan por `SLGTC`.
> - **H73 [ðŸ”´ SECURITY]** â€” **Arbitrary-program-exec auth review** (claim #219): `SE38`/`SA38`/`SA39` by end users â†’ review `S_DEVELOP` ACTVT=16.
> - **H74 [âœ… ANALYZED 2026-06-22 â€” hypothesis REFUTED]** â€” ~~Satellite TCP connectivity~~. Deepened (`knowledge/h74_syslog_10054_connectivity_analysis.md`, `process_mining/parse_syslog.py`, claim #236, supersedes #232's satellite part): **the 272 `10054` are NOT satellite drops.** 269/272 = end-user **SAP GUI frontend resets** (WP=DP dispatcher; `dpTermin` names workstations; 86% business-hours, weekday, Sat=1/Sun=8 = human curve). Only 3/272 = SAPâ†”SQL Server, all at **Sun 02:53 maintenance window**. **Zero gateway/RFC-server resets** â†’ MuleSoft/ORION RFC links not TCP-dropping. Remaining actions (â†’ fold into H78/monitoring): make SQL-Server alerting maintenance-window-aware; investigate `RFFMAVC_OVERALL_VIEW` batch SQL error; reclassify frontend churn as non-failures; keepalive-to-satellites NOT indicated.
> - **H75 [ðŸŸ¡ INTEGRATION â€” live break, re-flag of H52]** â€” **9,242 status-29 PROJECT IDocs** to MULE_PROD (CJ20N Core Manager integration broken). Escalate to the integration team.
> - **H76 [âœ… DONE 2026-06-22]** â€” **AGR pull from P01** done via `Zagentexecution/tasks/2026_06_22_h71_sod_write_channel/pull_agr_sod.py` â†’ Gold DB `agr_users` + `agr_1251_sod` + `agr_sod_map.json`. Unblocked H71. Method note: P01 secured `RFC_READ_TABLE` (SAIS) rejects `IN(...)` ("suspicious WHERE") â†’ read per-value equality. Declared-vs-actual finding surfaced: S_STANTIC does vendor change with NO F_LFA1 grant (custom-BAPI bypass). Remaining: USR02 (last-logon / right-unused) not yet pulled; broaden AGR scan beyond the 7 SoD users for H73 S_DEVELOP review.
> - **H77** â€” **OCEL with `origin=resource` per event** â†’ integration-first conformance (P2P process attributed to satellites). EKBE conformance is already channel-agnostic so findings hold; this adds the channel/resource axis.
> - **H78 [ðŸŸ¡ PARTIAL 2026-06-22]** â€” **operationalize the PROBLEMS accumulator**. âœ… CENTDATA parse DONE: `process_mining/parse_syslog.py` decodes the fixed format + classifies each row by channel (frontend_DIAG_reset / db_connection_reset / rfc_cpic_error / http_idle_logout / orion_app_error / batch_sql_error) â†’ `syslog_parsed_summary.json`. Remaining: wire BOTH `accumulate_problems.py` (weekly, SM21 ~7d retention) AND `parse_syslog.py` into the schedule so a >7d baseline accumulates AND the noise filter reclassifies frontend churn as non-failures (drop it from the "network error" signal â€” see H74).
> - **H79 [ðŸŸ¢ ECOSYSTEM]** â€” **align our 17 MuleSoft flows â†” PPM `INT-01..06`** with `unescore20-PPM-brain` (the other bridge end; `BROADCAST-ASRUN-MODEL` sent).
> - **H80** â€” **channel-mix unified** complete (proper IDoc/Jobs by-month, honest multi-unit view).
> - **H81 [ðŸŸ¢ ARCHITECTURE â€” the culmination]** â€” **Reconstruct the complete integration architecture** (the authoritative integration model). Fuse THIS session's audit-log-derived operating model (satellites named by host: MuleSoft synctrigger 17-flow PPMâ†”SAP sync, BRIDGE-RFC=ORION EAI portal, named-user write channel, WF-BATCH, PBC; read/write per satellite; 80% external) WITH the existing integration-element audit (H52: COUPA/GEODIS/SISTER/SuccessFactors/TULIP/UNESDIR/MuleSoft partner profiles + EDIDC/RFCDES/DBCON) INTO one per-integration model â€” for each integration: external system, channel (RFC/BAPI/IDoc/file/DBCON), FMs/BAPIs, direction (read/write), volume, **and failures** (9,242 status-29 IDocs H75, ~~the 10054 connectivity resets H74~~ â†’ CORRECTED: H74 shows the 10054 are frontend churn, NOT satellite failures; the real integration-failure tail is small â€” see H82/H83). Output: an **integration companion** (`companions/`) + domain doc (`knowledge/domains/Integration/`), aligned to PPM `INT-01..06` (H79). Sources: `system_operating_model_rfc.md`, `process_mining/rfc_process_classifier.py` (origin axis), `memory/project_integration_elements_gap.md` (H52). This is the end-to-end integration x-ray UNESCO actually runs on.
> - **H82 [ðŸŸ¡ OPS â€” from H74]** â€” **SAPâ†”SQL Server link: maintenance-window-aware monitoring.** The ONLY genuine backend TCP reset is the SQL-Server `dbsh` link (3Ã— `10054` + the lone `DBIF_REPO_SQL_ERROR` dump on `MDS_CTRL_STRATEGY`), all clustered at **Sun 2026-06-21 02:53** = one maintenance window, episodic not chronic. Activity: confirm the backup/maintenance window with BASIS/DBA; make any DB-connection alerting window-aware (suppress Sun 02:00â€“06:00) so it doesn't page on the known blip. Evidence: claim #236, `knowledge/h74_syslog_10054_connectivity_analysis.md`. (Needs: BASIS/DBA input â€” outside our read-only P01 reach.)
> - **H83 [ðŸŸ¡ OPS â€” from H74]** â€” **Investigate `RFFMAVC_OVERALL_VIEW` batch SQL errors.** The one *recurring* integration-adjacent fault: 3Ã— `DBSQL_*_ERROR` inside the FM Availability-Control overall-view rebuild batch job (00:00 / 05:00 / 07:30 windows). Not the network â€” check the job's SQL / locking / data volume. Evidence: claim #236, `knowledge/h74_syslog_10054_connectivity_analysis.md`. (Investigable from our side: trace the program + SM37 run history once VPN/P01 is up.)

> **â®• INCOMING (2026-06-22 â€” Gold-DB refresher industrialized + extractor maturity model; steward S-089).** Done-since: **PSM_FM + PS fully refreshed & at L4** (master/text/totals/transaction, P01â†’canonical Gold DB, delta-aware). New machinery: `brain_v2/gold_table_registry.json` (all 317 tables classified by **domain Ã— table_type**; builder `build_gold_table_registry.py`; human map `knowledge/gold_table_domain_map.md`) Â· `scripts/extraction/gold_refresh.py` (registry-driven, delta-aware: master=pk-upsert, totals=value-compare, txn=partitioned full-row, `_gold_sync_log` audit) Â· `brain_v2/gold_extractor_maturity.py` (per domain Ã— per table ladder L0â†’L4, wired into meta-capability EXTRACT). **Extractor maturity baseline = 29.1%** (PSM_FM 0.42 / PS 0.81; everything else L1). Caught+fixed a data-loss bug (fmioi key-collapse 1.81Mâ†’1.03M â†’ restored 2.19M, verified == P01 raw). Commits ec62194â†’8190f3c.
> - **H85 [ðŸŸ¢ MODEL â€” U_USAGE per domain + deep-dive]** â€” **Build the U_USAGE (Execution & Usage Footprint) census for the other 14 domains.** New capability dimension #11 (2026-06-23, `knowledge/capability_U_USAGE_execution_footprint.md`): per domain WHAT executes / WHO / HOW (dialog/batch/RFC-BAPI/BDC) / WHEN / VOLUME, from `rsau_audit_history` + `tbtcp` + apqi. Only **PSM_FM=HAVE** (`brain_v2/fm_executed_census.json`: 322/1,331 tcodes live=13.2%; RFC channel = MULESOFT/BRIDGE-RFC + named-user writes E_SILVA/A_BARONE â†’ H71). All others=NONE â†’ U_USAGE theme=6.7%, model maturity 30.3â†’28.2%. Method = `process_mining/fm_executed_census.py` (swap the name filter per domain, or drive from B_CODE object set). Then **deep-dive per object** (purpose/owner/dead-vs-used/S4-disposition), volume-ranked. Commit 258a4d3.
> - **H84 [ðŸŸ¢ DATA â€” extractor maturity lever]** â€” **Curate the remaining domains into the extractor registry** (climb extractor maturity 29.1%â†’higher). All non-PSM_FM/PS domains sit at **L1 (classified only, no refresh contract)**: **Payment 68 Â· Config 50 Â· FI 38 Â· Logs 16 Â· Master_BP 14 Â· Procurement 12 Â· Controlling 7 Â· Transport 4 Â· Integration 4 Â· Travel 2 Â· Security 2 Â· Cross_Domain 1** tables. Per domain: add curated specs to `CURATED` in `build_gold_table_registry.py` (goldâ†”SAP, key, delta strategy, partition, value_fields/amounts), then `gold_refresh.py <domain>` to sync â†’ L4. **Rule (data-loss trap, learned this session): a line-item table whose stored columns lack the full unique key must NOT be key-deduped â€” store all rows; ALWAYS verify golden==P01 raw count after a bulk txn refresh.** Method per table: `process_mining/method_registry.py <TABLE>`. Provenance (`d01_/v01_`, 24) is excluded (not a P01-refresh domain).

> **â®• INCOMING (2026-06-23 â€” PSM-FM U_USAGE deep-dive; steward S-090).** Done-since: PSM_FM `brain_v2/fm_usage_deepdive.json` (15 object cards + synthesis) committed. Claims #249â€“#252 promoted. New actionables H86â€“H87:
> - **H86 [ðŸ”´ PSM â€” CORRECTED 2026-06-23: read AVC from the STANDARD, retire the hand-rolled detector]** â€” âš ï¸ **`committed_vs_available_detector.py` is REFUTED â€” do NOT wire it.** It re-derives AVC with an invented formula (fmifiit WRTTP 66âˆ’54 + bpjaâˆ’coepâˆ’cooi) and produces nonsense pools (fund **196EAR4042**: detector pool **âˆ’$12,940,635** vs the **standard AVC ledger FMAVCT net +$1,565,164** â€” wrong by $14.5M AND sign; total UNES 2026 net = **+$375.7M**, AVC holds). Same error class as GL-from-BSIS instead of GLT0. **The REAL AVC is computed by the system:** control object=(Fund,FundsCtr,CmmtItem), ledger **9H** active all 9 institutes (tolprof 9HZ00001; ICTP=ZIT1/UBO=Z002/UIL=Z001/rest=Z000), activity groups `fmavcatgr` map consumption WRTTPâ†’amount type (50â†’20/51â†’30/54â†’40/60-61â†’40), available=consumableâˆ’consumption, blocks at posting. **DO:** read real AVC via the STANDARD â€” report **FMAVCR / RFFMAVC*** or the standard **FMAVC read FM** on P01, OR re-extract **FMAVCT fully** (our slice has only ALLOCTYPE_9=KBFC). The real blind-spot test = compare YPS8/YFM1 output vs the STANDARD FMAVCR, not vs the broken detector. Claim #249 quantification SUPERSEDED; YPS8/YFM1 reuse-the-filter (code, #153/#159) still stands but needs re-validation against the standard. See [[feedback_avc_real_from_standard_not_handrolled]].
> - **H87 [ðŸŸ¡ PSM / S4 â€” ICTP monolith consolidation scoping]** â€” **Scope and plan ICTP custom suite decommission / consolidation onto standard tcodes.** `ZICTP_COCKPIT` (5,014 / 8 users, 42% on D_CRESEVICH+B_RAZZINI) fronts 70+ ZICTP_* objects re-implementing ME28/ME2L/MIRO/FBL1N/cash/VAT for one Cat-1 institute (Trieste). Largest single S/4 re-platform/decommission scope in PSM_FM + clearest bus-factor risk (claim #250). Step 1: inventory ZICTP_* package via TADIR + extract all program sources. Step 2: map each ZICTP_* to its standard-SAP equivalent (ME28â†’standard ME28, ZICTP_REP_BCSâ†’RFFMEPGAX/FMRP_RFFMEP1AX). Step 3: gap assessment with ICTP local team. Cross-links: claim #218 (cockpit code), claim #251 (fork duplication), R_S4_READINESS PSM_FM.

> **â®• INCOMING (2026-06-23 â€” "how UNESCO works": map every executed object; discovery PATTERNS).** Goal restated by user: explain **100% of transactions/reports/RFC-BAPIs/jobs that run** â€” what they are, who/what calls them, where from, and which are integrations. Done-since: built `process_mining/executed_objects_domain_map.py` + `brain_v2/executed_objects_domain_map.json` (objectâ†’domain map, 4 channels), cached **`tadir_prog`(388K)+`tdevc`(28K)** in the Gold DB (durable), wrote `knowledge/operating_model_discovery_methods.md` (the 4 mapping methods + hidden-extraction + time-factoring patterns). New actionables H88â€“H90:
> - **H88 [ðŸŸ¢ MODEL/DISCOVERY â€” complete the executed-objectsâ†’domain map]** â€” The map currently reaches **60% by execution volume / 39% by object** via the PACKAGE method (TADIR.DEVCLASS). Raise it by triangulating the other 3 methods (`operating_model_discovery_methods.md`): (a) **objects-read** â€” resolve generated programs by their embedded object (`/1BCDWB/DB<table>`â†’tableâ†’domain, SAP-Query `AQ*/!Q*`â†’workspace/table); (b) **callerâ†’domain** â€” map service-accounts + dialog users to domains + actor-type (human/integration/batch); (c) extend DEVCLASS/name rules for the ~670 custom Z/Y + ~1,051 std missed. **Honest scope:** the tail is ad-hoc queries + generated + technical substrate (SAPMSSY1/RS*) â€” the technical substrate is a legitimate non-business tier, not "lost knowledge". This is the **U_USAGE** method at scale (folds into H84/H85). Artifacts committed this session.
> - **H89 [ðŸ”´ GOVERNANCE/SECURITY â€” ungoverned ad-hoc HR data extraction (hidden extractions)]** â€” VERIFIED 2026-06-23: **6,060 ad-hoc SAP-Query executions Â· 1,798 distinct queries Â· 153 users; 60% are HR (3,658 execs, 39 users)** = sensitive personnel data extracted outside governed reports. Top runner **JOBBATCH (1,890) = queries SCHEDULED AS JOBS â†’ automated extraction â†’ almost certainly file output = a parallel integration nobody catalogued.** Active & growing (2026-03â†’06, ~1,500/mo). **DO:** (1) trace which HR infotypes/tables the top HR queries read (what data leaves) + who; (2) trace whether the JOBBATCH-scheduled queries write to file system (OPEN DATASET) â€” confirms shadow integration; (3) route as a data-governance finding. Cross-links: H73 (arbitrary-extraction auth), H71 (write-channel SoD). Pattern doc: `operating_model_discovery_methods.md`.
> - **H90 [ðŸŸ¡ MODEL â€” integration layer + TIME axis on U_USAGE]** â€” Add the BEHAVIOR axis (read / DB-write / **file** OPEN-DATASET / RFC-out) + ACTOR axis (human / integration MULESOFTÂ·BRIDGEÂ·UBO-RFCÂ·SISTER / batch) to every executed object, so "internal process" vs "integration" is separable system-wide (integration = technical caller OR file OR write/call-out). Add the **TIME** axis (when used, monthly) to detect active-vs-dead + seasonality. First probe = the H89 queryâ†’jobâ†’file trace. Feeds F_INTERFACE_FILE + R_S4_READINESS (dead-code).

> **â®• INCOMING (2026-06-18, worked via the unesco-sap-brain harness â€” session migrated here, NOT yet reconciled):** BCM signatory **node-selection mechanism FOUND = infotype 1218** (`HRP1218`/`HRT1218` expressions on `BNK_STR_BATCH_REL_APPR`: ZBUKR + amount band, bank-agnostic; `HRP1222` empty) + **INC-000011781** (UBO / Renata Ritter, SPEC_READY: ADD 10021811 Ã—4 nodes, DELIMIT Martin 10108464 from 50034893; Ba/Carvalho/Yli-Hietanen parked for TRS). Landed: `knowledge/domains/Treasury/bcm_signatory_change_solution_design.md`, `companions/bcm_signatory_companion.html`, `knowledge/incidents/INC-000011781_ubo_bcm_add_ritter.md`, Treasury/README "BCM Signatory Panel Management (hub)", IT1218 section + change-history in `bcm_signatory_rules.md`, extended `feedback_bcm_signatory_intelligence.md`, skill triggers. ~~**TODO: `python brain_v2/rebuild_all.py`** to fold IT1218 + INC-000011781 into `brain_state.json`~~ â€” **DONE s099 (2026-08-17), but the TODO itself is the lesson.** It sat unexecuted from 2026-06-18 to 2026-08-17: INC-000011781 (our richest BCM precedent â€” IT1218 node selection, the drift sweep, the role gap) had a doc on disk and **no first-class record**, so `BRAIN LOOKUP` could not reach it. Same for INC-180995 and INC-CLASS-LOSS-2026-06 â€” **11 docs vs 7 records**. All 3 backfilled; `incidents.json` 7â†’10, brain 1634â†’1667 objects, coverage held at 100.0%, blind_spots 0. **A TODO is not a control** â†’ the control now exists: `Zagentexecution/quality_checks/incident_record_coverage_check.py` (exit 0 = clean) + rule `feedback_incident_doc_without_record_is_invisible`.

---

## ðŸ”´ INC-PSTLADR-NOV2026 â€” GRAVE, deadline duro 14-11-2026
...
no tendran de donde sacarlo. **NO son los proveedores** â€” FI-AP esta al 100% con ciudad; la averia
...
**Registro completo de errores y acciones:** `knowledge/domains/Payment/dmee_map/REGISTRO_ERRORES_Y_ACCIONES.md` â€” 17 riesgos medidos, con los
descartados y la medida que los descarto, para que no se reabran.

**Por orden:** B1 cargar los 399 proveedores US/CA (fichero listo, `FIXB_1_CARGAR_region.csv`) Â·
F1 meter `pain001_address_validator.py` en el ciclo ANTES del envio Â· A3 encender `REACT_LEV`
...
**Corregido 2026-08-19:** la nomina NO esta en alcance â€” los 818 sin ciudad cobran por cheque.
El alcance real son los proveedores: 8.149 sin `CtrySubDvsn` en TODOS los rails.

Doc: `knowledge/incidents/INC-PSTLADR-NOV2026_direccion_estructurada_nomina.md` Â· claims 504-509
Drill: `python brain_v2/load_domain.py dmee`

## PROJECT BRAIN â€” What We Built

### The System (as of 2026-03-26)

```
UNESCO SAP Intelligence Toolkit â€” 10 Capability Layers, 38 Skills
â”œâ”€â”€ L1: SAP Connectivity      â†’ pyrfc + SNC/SSO (D01 dev / P01 prod)
â”œâ”€â”€ L2: Data Extraction       â†’ ~2.5GB gold SQLite DB, 24M+ rows, 68 tables
â”œâ”€â”€ L3: Validation/Domain     â†’ sap_brain.py (73.9K nodes, 3-level access), 4 domain agents + coordinator
â”œâ”€â”€ L4: Code Extraction       â†’ ADT API, BSP/OData/Enhancement extraction
â”œâ”€â”€ L5: Transport Intel       â†’ CTS dashboard, 7,745 transports analyzed
â”œâ”€â”€ L6: Fiori Development     â†’ Offboarding clone (React+UI5 Web Components)
â”œâ”€â”€ L7: Process Intelligence  â†’ process-intelligence.html + pm4py (848K P2P events)
â”œâ”€â”€ L8: System Monitoring     â†’ sap_system_monitor.py (SM04/SM35/SM37/ST22)
â”œâ”€â”€ L9: Class Deployment      â†’ 16 scripts (create, deploy, verify ABAP classes)
â”œâ”€â”€ L10: BDC Intelligence     â†’ bdc_full_inventory.py (Allos/Y1 payroll forensics)
â””â”€â”€ L11: Integration Intel    â†’ 38 systems, 334 RFC FMs, 7 UNESCO .NET apps mapped
...
Each layer FEEDS the others:
  L2â†’L3 (data validates domain), L5â†’L4 (transportsâ†’code targets),
  L3â†’L6 (domain rulesâ†’app logic), L4â†’L6 (codeâ†’rebuild),
  L2â†’L7 (event logs), L5â†’L7 (CTS data)
```
...
| `PRAAUNESC_SC` BDC â€” 89 sessions by `_COLLOCA` | #1 Allos replacement target |
| `OBBATCH` â€” 109 automated sessions | Background automation via BDC |
...
| FMIFIIT.KNBUZEI = BSIS.BUZEI | Perfect 1:1 line-level FMâ†”FI join |
| FMIFIIT.OBJNRZ = PRPS.OBJNR | Proper WBS element recovery (85.9% coverage for 2025) |

### What the Brain Knows (73,877 nodes â€” Session #014)
...
- **5 PROCESS nodes**: B2R, H2R, P2P, T2R, P2D â€” UNESCO core processes
...
- **Coordinator**: `.agents/skills/coordinator/SKILL.md` â€” routes by process type
- **PSM Domain Agent**: `.agents/skills/psm_domain_agent/SKILL.md` â€” FM/budget
- **HCM Domain Agent**: `.agents/skills/hcm_domain_agent/SKILL.md` â€” HR/employee lifecycle
- **FI Domain Agent**: `.agents/skills/fi_domain_agent/SKILL.md` â€” GL/validation/substitution

---

## PMO BRAIN â€” Single Source of Truth for ALL Pending Work
...
### ðŸ”´ BLOCKING â€” Cannot progress without these
...
| ~~B2~~ | ~~BSEG PROJK extraction~~ | ~~#016~~ | ~~WBS for ~9.5% non-FM docs~~ | ~~Closed #035: BSEG is not a table â€” it's a JOIN (bseg_union VIEW). Golden Query covers 85.9% WBS via FMIFIIT.OBJNRZâ†’PRPS. Remaining 9.5% = clearing lines without FM match, marginal value. Resolved by design.~~ |
| ~~B3~~ | ~~CO tables: COOI, COEP, RPSCO~~ | ~~#005~~ | ~~Entire CO cost layer missing~~ | ~~Done #035: 3,451,708 rows (COOI 773K + COEP 2.55M + RPSCO 127K). DD03L-verified fields. Period-by-period extraction for VPN resilience. Gold DB loaded. Anchor estimate was 1.6M â€” actual 2x larger.~~ |
| ~~B4~~ | ~~B2R tables: FMIOI+FMBH+FMBL verification~~ | ~~#009~~ | ~~B2R lifecycle mining~~ | ~~Done â€” verified #028: FMIOI=1.8M, FMBH=287K, FMBL=319K rows~~ |
| ~~B5~~ | ~~SES gap: ESSRâ†”ESLL PACKNO mismatch~~ | ~~#011~~ | ~~P2P service receipts~~ | ~~Done â€” verified #028: 707K PACKNO matched (99.99% of ESSR)~~ |
...
| ~~B7~~ | ~~CDHDR process mining~~ | ~~#011~~ | ~~Audit trail analysis~~ | ~~Done â€” verified #028: cdhdr_activity_mapping.py exists with pm4py integration~~ |
| ~~B8~~ | ~~P2P bottleneck/temporal analysis~~ | ~~#009~~ | ~~P2P insights~~ | ~~Done â€” verified #028: p2p_process_mining.py + HTML dashboard built~~ |
...
particionarlo por dominio Â· dejar de materializar en el las capas que ya viven en su store Â·
...
**H110 â€” EL ACUMULADOR PIDE SIEMPRE 16 DIAS, PASE LO QUE PASE: 45 DIAS DE SM20 AUSENTES (s102, medido 2026-08-22).**
...
**julio entero = 0 filas** (`202602` 299K Â· `202603` 3,49M Â· `202604` 4,94M Â· `202605` 4,30M Â·
`202606` 2,58M Â· **`202607` 0** Â· `202608` 2,22M).

**No estaban perdidos â€” sonda a P01 el 2026-08-22, ventana 08:00-10:00h:**
20260622 â†’ 23.076 filas Â· 20260701 â†’ 21.793 Â· 20260715 â†’ 21.117 Â· 20260731 â†’ 20.865 Â·
20260805 â†’ 22.113 (control: 20260810 â†’ 22.987, 20260820 â†’ 19.689). O sea **la retencion real de SM20
en P01 es â‰¥ 62 dias**, no los ~14d que el script asume ni el â‰¥61d del claim #212. El hueco era
...
medida â€” nunca una constante. El `freshness_guard()` ya DETECTA el gap y lo imprime como WARN; luego
...
(70d), no de "dias desde la ultima corrida" â€” porque una corrida que muere a medias sella el estado
...
decia "lost forever" y era falso. (d) **RSAU no salia en el informe de cobertura** â€” por eso el
...
accion, es el coste ya pagado de haber tenido este defecto abierto â€” y la medida de por que la
cobertura tiene que mirarse por dias y no por span.

**H109 â€” UN REBUILD SOBREVIVE AL SUSPEND Y COLISIONA CON EL SIGUIENTE (s102, medido 2026-08-22).**
...
lanzo dos a sabiendas â€” uno resucito. Una regla que depende de que el operador recuerde no cubre el
caso en que el sistema recuerda por el.

**Mecanizado el 2026-08-22 â€” `brain_v2/rebuild_lock.py`, cableado en `rebuild_all.py`:**
1. âœ… **Lock con PID y heartbeat**. HELD se niega y dice quien manda Â· ORPHAN (PID muerto) lo
   reclama solo Â· HUNG (vivo pero sin latir >15 min) se niega y **no mata**. Probado en los tres.
2. âœ… **Tope de duracion** â€” >45 min cuenta como HUNG.
3. â¬œ **Guard al commitear** â€” la funcion existe (`is_rebuild_running()`), **falta llamarla desde el
...
**H108 â€” 11 DE 13 INCIDENTES NO DEJARON PROCESO, SOLO CASO (s102, medido).** Lo detecto JP:
...
**Tres pares â€” dos incidentes del mismo dominio y tipo â€” donde el proceso YA se puede escribir:**

| Dominio | Tipo | Incidentes | Proceso que falta |
|---|---|---|---|
| **BANCOS** (hoy tagueado `Treasury`, canonico `Treasury_EBS`) Â· eje **P2P** | MASTER_DATA | `INC-000006313` + `INC-000011781` | **panel de firmantes BCM**: alta y baja de personas, la carta como autoridad de registro, el barrido de la poblacion. Correccion de JP: BCM vive en el dominio de los BANCOS y su eje es purchase-to-pay, no T2R â€” autorizar un pago es P2P. El canonico `Treasury_EBS` induce a error: ese dominio es banco casa **y** extractos |
...
| `PSM_FM` â€” **Fund Management** | ERROR | `INC-000005638` + `INC-BUDGETRATE-EQG` | **tipo de cambio presupuestario y deriva del pool de AVC** |
...
*Siguiente accion: escribir el proceso de autorizacion bancaria BCM â€” es el par mas maduro, con
dos casos ya documentados en detalle.*

**H107 â€” 50 CUENTAS CON OB09 Y SIN VARIANTE DE F.05 (s102, medido). Escala muy superior a
...
configurado** â€” alguien quiso revaluarlas y nunca entraron en el calculo.

| Familia | Cuentas | Lo mas grueso |
|---|---|---|
| Travel Agency Clearing de oficinas de campo (`920x`/`923x`) | **31** | UZS 136,5 M Â· IRR 222 M Â· NGN 14,2 M |
| Clearing de institutos (`509x`) | **7** | `5098012` UIS en **13 monedas** Â· `5091011` EUR âˆ’12,89 M |
| Inversiones y depositos (`404x`) | 2 | `4041011` EUR 10 M Â· `4044015` EUR âˆ’3,97 M |
| Otras | 10 | `2111000` VAT HQ EUR 1,44 M Â· `4060000` Fixed Assets Clearing en 4 monedas |

**Metodo que hay que usar para revisarlo** (esta en el agente `fx-revaluation-scope`): la
incoherencia no es "no estar en una variante" â€” hay 497 cuentas de balance fuera y la mayoria con
razon â€” sino **no estar cuando tus iguales de la misma posicion de balance si estan**. Filtrado
asi: 208 cuentas.
...
**H106 â€” PREGUNTA A TESORERIA: 3 cuentas con OB09 y en ninguna variante de F.05 (s102, medido).**
...
(PIMCO Â· JP Morgan Â· RAMP). `4041019` NO entra: cerrada en INC-000016262 (USD sobre sociedad USD).
...
incluidas â€” `4043011` (USD, EUR 20,5 M), `4043025` (USD, EUR 7,3 M), `4043026` (USD, EUR 6,2 M).
**El dato que cierra la cuestion:** de las 25 cuentas de la familia, **8 tienen exposicion abierta
en divisa y SIETE estan dentro de la variante**. La unica fuera es `4041011` â€” 571,6 M USD de saldo
y 10 M EUR abiertos. Es una **omision suelta**, no un criterio mal planteado. La pregunta a
Tesoreria es directa: Â¿por que esta fuera, y se anade?

Instrumento: `python Zagentexecution/quality_checks/fx_revaluation_scope_check.py` Â· agente
`fx-revaluation-scope` Â· claims 558-561.

**H105 â€” REVALUACION FX (F.05 / SAPF100): el dominio NO existe, la materia SI (s102, medido).**
...
**VACIO** â€” 0 docs, 0 companions, 0 objetos, 0 incidentes, sin `coverage_pct` â€” mientras 4 docs de
...
**Defecto de datos a arreglar primero (barato y silencioso):** conviven **dos grafias** â€”
...
5 incidentes, y declarar la matriz **que cuenta se valora, con que metodo, por que variante** â€”
...
**H104 â€” MASTER DATA GOVERNANCE: explorar y expandir el dominio (s102, JP).** El dominio existe
...
1. **La cadena de tareas POSTERIORES de cada tipo.** Solo esta medida la de la cuenta de mayor
   (7 pasos). Para un centro de coste, un fondo o un PEP no sabemos que dispara un alta â€” y ese
...
4. **Ejecutores para los 7 tipos con canal medido y sin ejecutor** â€” centro de coste primero
...
*Instrumentos que ya existen:* `gl_alignment_check.py` Â· `ob09_vs_variant_check.py` (**A47 `DEFECTO_VIVO`** -- arreglar `main()` antes de apoyarse en su salida; claim 599) Â·
`fsv_coverage_check.py` Â· `config_transport_prerelease_check.py` Â· agente `master-data-sync`.
*Doc:* `knowledge/domains/Master_Data_Governance/gl_account_creation_process.md`.
*Reglas:* `feedback_a_master_data_domain_is_a_registry_of_object_types` Â·
`feedback_a_config_object_applies_to_a_population_prove_it_before_measuring`.


**H91 â€” SECURITY: open it as a DOMAIN (s097, JP).** It has never been one, and that is exactly
why searching for it returns nothing: today it exists only as capability column `E_AUTH`, empty
in 16 of 21 domains, and as the single MISSING component of the installation profile â€” nothing
...
**H92 â€” ATTRIBUTION: 5.3M calls that no person can be tied to (s097, measured).** The
"80% external" headline hides a sharper fact. Behind each technical account, `PARAMX` carries
the calling `user=` â€” and the accounts split cleanly in two:
- **TRANSPARENT** â€” `WF-BATCH` 660,588 calls with **542 distinct real users** behind it (only
  14% is the account itself), `SMTMSBP` with 4 technical users.
- **OPAQUE** â€” `MULESOFT` 3,230,958 calls, ONE identity, 100% itself. `BRIDGE-RFC` 2,106,347
  and `UBO-RFC` 324,390 carry **no `PARAMX` at all** â€” not even a caller field.
...
events) and 1,882 by RFC/CPIC. The finding is not "nobody uses SAP" â€” it is that **the write
...
**H93 â€” SUPPORT has no coverage metric (s099).** The Support domain registry now carries its
docs, incidents, two tracks, procedures and 4 recurring checks â€” but `coverage_pct` is still
...
**H94 â€” PERNRs are synthesized into the brain typed `GL_ACCOUNT` (s099, measured).** Drilling
...
objects) â€” H41 lists 11 PERNRs from blind_spots; Track B keeps adding more with every signatory
...
| ~~H2~~ | ~~Create `sap_process_mining` skill~~ | ~~#017~~ | ~~Skill~~ | ~~Done â€” SKILL.md exists, Functional (3). OCEL pending as G1~~ |
| ~~H3~~ | ~~Create `sap_change_audit` skill~~ | ~~#017~~ | ~~Skill~~ | ~~Done â€” SKILL.md exists, Functional (3). Compliance template â†’ added #023~~ |
| ~~H4~~ | ~~BSEG UNION view in SQLite~~ | ~~#011~~ | ~~Data~~ | ~~Done â€” bseg_union VIEW exists, 4.7M rows (BSIS+BSAS+BSIK+BSAK+BSID+BSAD). Verified #023~~ |
| ~~H5~~ | ~~Merge sap_segw + segw_automation~~ | ~~#018~~ | ~~Skill~~ | ~~Done #023 â€” sap_segw now comprehensive (5 workflows, element IDs, full troubleshooting). segw_automation redirects.~~ |
...
| ~~H8~~ | ~~P01 transaction usage report~~ | ~~#005b~~ | ~~Monitoring~~ | ~~Done â€” verified #028: sap_system_monitor.py --report transactions works~~ |
| ~~H9~~ | ~~P01 runtime dumps report~~ | ~~#005b~~ | ~~Monitoring~~ | ~~Done â€” verified #028: sap_system_monitor.py --report dumps works~~ |
...
| ~~H11~~ | ~~**Extract Benefits BSP + HCM Z-reports (merged H11+H12)**~~ | ~~#005b (merged #036)~~ | ~~Code~~ | ~~**Done #038** via RFC (not ADT â€” avoided the .env password dependency). 4 HCMFAB MYFAMILY classes extracted: `ZCL_ZHCMFAB_MYFAMILYME_DPC` + `_DPC_EXT` + `_MPC` + `_MPC_EXT`. 7 BSP apps discovered in ZFIORI package: YHR_BEN_ENRL (Benefits Enrollment), YHR_EDURENT_APV, YHR_OFFBOARDEMP, YHR_OFFBRD, ZHRBENEFREQ, ZHREDURENTALADM, Z_HCMPROCES_EXT. 222 HCM namespace Z-reports discovered, 12 extracted as sample (including ZZHRPAF02 186KB, ZHRAUTOSTEP, ZNHR_LOAD_PAYSCALE_GROUPS). Channel: `READ REPORT` via `RFC_ABAP_INSTALL_AND_RUN` over SNC/SSO â€” same channel as H29.~~ |
| ~~H12~~ | ~~Populate HCM/Reports folder~~ | ~~#005b~~ | ~~Code~~ | ~~MERGED into H11 #036.~~ |
| ~~H13~~ | ~~**BCM dual-control gap remediation**~~ | ~~#021/#027~~ | ~~Audit~~ | ~~**Done #037 â€” Deliverable 1 shipped.** Monitor script `Zagentexecution/bcm_dual_control_monitor.py`, HTML companion `bcm_dual_control_audit.html`, executive summary `knowledge/domains/BCM/h13_executive_summary.md`. Reframe: automation debt, not fraud. 3,359 same-user batches in scope / $656M exposure. Top 2 = C_LOPEZ + I_MARQUAND (94.7% / 92.9% self-approval) â€” HQ treasury manual Wednesday cycle, no 3rd operator. F_DERAKHSHAN reclassified (74% dual-controlled). +1,366 drift since #027 (15 sessions of inaction cost). Paths 2â€“5 (carve-out, role split, workflow mod, automation) spawn as H13a/b/c/d in next review.~~ |
| ~~H14~~ | ~~**Extract YWFI package source from D01**~~ | ~~#021~~ | ~~Code~~ | ~~**Done #038** via RFC (TADIR query DEVCLASS='YWFI' â†’ 37 objects). Full extraction: classes, programs, FUGRs with method includes. Key find: `ZWF_GET_CERTIFYING_OFFICER` (NOT `Z_WF_GET_CERTIFYING_OFFICER` as PMO said â€” wrong underscore) is a FUGR with 5 includes (TOP+UXX+U01+U02+U03, 7KB of logic). Other extracted FUGRs: `ZFI_PAYREL_EMAIL`, `Z_WF_FI_EVENT_PAYMENT_METH`, `Z_WF_FI_EXCLUDE_NOTIF`, `Z_WF_FI_GET_CLASSIC_VALID`. Programs: `YBSEG_REL`, `ZNOTREJECT`. Output: `extracted_code/YWFI/`.~~ |
...
| ~~H21~~ | ~~**Bank recon amounts: currency conversion**~~ | ~~#028~~ | ~~Analytics~~ | ~~**Done #042.** CURRENCY_USD_RATES table (203 currencies from TCURR type M) + FEBEP_USD view created in Gold DB. Real USD total: **$16.8B** (not $13.9B â€” conversion increased total). UZS=$6.2B (37% from 1.3% items), USD=$5.8B, EUR=$2.7B. Weak currencies (UZS/IRR/LBP) inflate, don't deflate. Falsified hypothesis that DMBTR was inflated.~~ |
| ~~H22~~ | ~~**FEBEP full fields extraction**~~ | ~~#029~~ | ~~Data~~ | ~~**Closed #042 â€” DATA WAS ALREADY COMPLETE.** PMO was stale 12 sessions. `FEBEP_2024_2026` table has 223,710 rows with 27 fields, ALL months present (2024-01 through 2026-03). The unsuffixed `FEBEP` (50K rows, 104 cols) was the truncated first attempt â€” the real data lives in the `_2024_2026` table. "Missing months" claim was wrong.~~ |
| ~~H23~~ | ~~**FEBKO full fields extraction**~~ | ~~#029~~ | ~~Data~~ | ~~**Closed #042 â€” DATA WAS ALREADY COMPLETE.** PMO said "Missing HBKID" but `FEBKO_2024_2026` has 31,416 rows with 62 fields INCLUDING HBKID (0% null). The unsuffixed `FEBKO` (50K rows, 8 cols) was the truncated first attempt. Same pattern as H22 â€” always check `_2024_2026` suffix.~~ |
...
| ~~**H34**~~ | ~~**Code Brain v2: SAP cross-reference extraction + annotation framework + incident linking**~~ | ~~**#048**~~ | ~~**Brain**~~ | ~~**Done #049 â€” pivoted from extraction to architecture rewrite.** D010TAB/WBCROSS = 200M+ rows, never bulk extract. Built Brain v3 hybrid instead: object-centric `brain_v2/brain_state.json` (4.2% of context, 10 AGI layers, 1 Read = full intelligence). 50 feedback rules migrated from `~/.claude/memory/` (portable). 26 claims (15 superseded for anti-regression). 19 known_unknowns + 16 data quality issues mined from 48 retros. Single-command rebuild via `rebuild_all.py`. SessionStart hook in `.claude/settings.json`. AGI score 41/100â†’90/100. See `Brain_Architecture/brain_design_specification_v3.md` and `session_049_retro.md`.~~ |
| ~~**H35**~~ | ~~**Validate Brain v3 in real session work**~~ | ~~#049~~ | ~~Brain~~ | ~~**Done #050.** Brain v3 used for the entire session including a self-evaluation that produced 12 deliverables. FALS-003 falsified (I globbed before reading the brain link) â€” fix shipped: `feedback_brain_first_then_grep` rule + Layer 11 (incidents) + Layer 12 (blind_spots) + `_coverage` metric + force-include. Object count 102â†’136. AGI score 90â†’101 (added Failure-mode visibility criterion).~~ |
| **H36** | **Triage brain blind_spots â€” superseded by H50** | #050 | Brain | Original scope #050: 20 blind_spots. Session #054 audit: count grew to **71** (all MISSING flavor). Coverage dropped 75.6% â†’ 64.3%. H36 is preserved as history anchor; **H50 supersedes with updated scope**. H36 action list remains valid for the first 20: extract PA0027, ZCL_IDFI_CGI_DMEE_FR, ZCL_IM_TRIP_POST_FI_CM006; model GB901/GB922 from Gold DB; drop pseudo-refs (LHRTSF01:852, inst_fund_ba_wbs_cc, GB901:2IIEP###001). |
...
| **H43** | **Register 4 missing objects from session #053 (FMAVCT, KBLEW, CL_FM_EF_POSITION, /SAPPSPRO/PD_GM_FMR2_READ_KBLE)** | #054 | PSM/Brain | Session #053 retro Â§11 committed to registering these as first-class brain objects â€” never done. All 4 confirmed in `brain_state.blind_spots`. Required fields: type (TABLE/CLASS/FUNCTION_MODULE), domain (PSM), annotations with Session #053 findings (FMAVCT wide-table query pattern, KBLEW cluster wrapper, CL_FM_EF_POSITIONâ†’CHECK_CONSUMPTION as H39 fix target). Source data: retro Â§6 "Tables/data sources discovered" + Â§7 "Custom code/class architecture". |
| **H44** | **Build `sap_fm_avc_intelligence` skill (AVC = core PSM)** | #054 | PSM/Skill | Availability Control is the foundation of PSM at UNESCO. Skill must encapsulate: FMAVCT wide-table extraction pattern (narrow FIELDS, split OPTIONS), cluster-table access via `/SAPPSPRO/PD_GM_FMR2_READ_KBLE`, fund-level vs FR-line-level metric distinction, Camp A/B/C enhancement asymmetry classification for BR composites, EURX rate type handling, FMIOI WRTTP=81 carryforward pair interpretation (CP-003: sample before aggregating), FMAVCT dedup by RVERS/RPMAX. Source references: retro #053 + `knowledge/domains/PSM/EXTENSIONS/budget_rate_custom_solution.md` + `Zagentexecution/quality_checks/budget_rate_consumption_audit.py`. Deliverable: `.agents/skills/sap_fm_avc_intelligence/SKILL.md` + MATURITY entry. Upgraded from "consider" (retro Â§13) to HIGH based on domain criticality. |
...
| **H63** | **Build a `Transport_Intelligence` companion (domain exists only as Word)** | #083 (2026-06-21) | Companion/Viz | The repo-wide 100% file census surfaced a knowledge domain with **no HTML companion** â€” it lives only as 5 `.docx` under `knowledge/domains/Transport_Intelligence/` (`SAP_Transport_Intelligence_Reference.docx`, `_Modules_Supplement.docx`, `_REFX_Dunning.docx`, `_Workflow.docx`, `_FlexWF_InvUnblock_BCM.docx`). Action: synthesize an HTML companion from those docs (artifact-hierarchy: Word is the snapshot, the companion is the living layer), register in `companions/companions.json` (domain `transport`), let `build_companion_graph.py` relate it. NOTE: `transport_companion_D01K9B0CBF_v2.html` already exists but is a single-transport (STEM) companion, NOT the domain reference. |
| **H64** | **Classify/register `my_monitors_dashboard.html`** | #083 (2026-06-21) | Companion/Decision | Census found the only genuinely-unregistered dashboard: title "My Monitors â€” JP Lopez Personal Dashboard" (`Zagentexecution/my_monitors/my_monitors_dashboard.html`, git-tracked, 8KB). Decide: (a) promote â†’ copy to `companions/`, register in `companions.json` under a new `monitoring` domain (the landing `domains_map` catch-all already renders unmapped domains, so it'll appear); or (b) keep as a personal/local tool, out of the registry. The two `golden_*_rows.html` viewers are raw Golden-DB row dumps (no title) â†’ leave as data viewers, NOT companions. |
| **H65** | **Clean up ~20 stale git-tracked duplicate companion HTML** | #083 (2026-06-21) | Companion/Cleanup | `companion_graph.json â†’ html_inventory_repo_wide` lists deterministically: 3 `duplicate_copies` (identical) + 17 `diverged_stale` (OLDER generations) of the canonical `companions/*.html`, in build dirs (`Zagentexecution/mcp-backend-server-python/`, `Zagentexecution/dashboards/`, `Zagentexecution/sap_data_extraction/reports/`, repo-root `py_finance_wage_type_companion_v1.html`). `companions/` is the current copy (verified by mtime). These tracked stale copies create the "more companions" illusion and risk opening an old version. Action (needs user OK â€” `git rm` of tracked files): remove stale copies, point build scripts to emit directly into `companions/`, re-run `build_companion_graph.py` to confirm 0 duplicates. Regenerate the list anytime: `python scripts/build_companion_graph.py`. |
| **H52** | **Integration-element audit â€” per-integration SAP elements (FM / IDoc / program+variant / endpoint)** | #055 | Integration/Brain | **MAJOR PROGRESS session #055**. Live probes on BOTH D01 and P01 via pyrfc + probe script `probe_p01_integration.py`. **5 program sources extracted** (COUPA â†’ `YCL_FI_COUPA_ACCOUNTING_LOAD` + RFBIBL00; GEODIS = SFI CSV; SF_ORGANIZATION = SFI org; TULIP = DBCON pull; UNESDIR = ADâ†’PA0105 subtype 0010). **MuleSoft partner profile on P01** (EDP13 outbound: PROJECT/PROJECT02, FUNDSCENTER/FUNDSCTR, SYNCH; EDP21 inbound: ADRMAS/ADR2MAS/ADR3MAS/DEBMAS). **P01 live EDIDC**: **9,242 PROJECT IDocs to MULE_PROD, ALL failing at status 29 (ALE service error)** â€” recent example DOCNUM 4143503 created 2025-11-12 by user M_SARMENTO-G via CJ20N. FUNDSCENTER + SYNCH profile entries have zero traffic. **DBCON P01 = 2 entries only** (TULIP + UNESDIR); SISTER DBCON exists only on D01 â†’ SISTER production is RFC-only. **Corrections vs prior memory**: (a) SuccessFactors IS LIVE; (b) GEODIS = SFI not logistics; (c) SISTER = RFC-only in P01 (not 2-channel); (d) UNESDIR = SQL-direct not HTTP; (e) P01 â‰  D01 partner profiles (drift). **New ops finding (flag to team)**: 9,242 accumulated status-29 PROJECT IDocs â†’ CJ20N Core Manager integration is broken. **Remaining unknowns**: Salesforce BOR mechanism (no P01 evidence); .NET Web.config â†’ FM mapping; COUPA file path (inside class); UNJSPF SOAP payload. Full inventory: `memory/project_integration_elements_gap.md`. |
| H25 | **T028A + T028E extraction (account symbol definitions)** | #029 | Config | T028A = symbol-to-GL mapping (BANKâ†’10xxxxx, BANK_SUBâ†’11xxxxx). T028E = posting key definitions. Validates the account symbol configuration. **Zombie age 19. Kill if not done by #055.** |
| H26 | **T012K UKONT re-extraction** | #029 | Config | T012K missing UKONT field (sub-bank GL paired with bank GL). Need to re-extract with all fields to validate 10xxxâ†”11xxx pairing. **Zombie age 19. Kill if not done by #055.** |
| ~~**H33**~~ | ~~**Brain v2: Absorb ALL text into graph â€” zero dead text**~~ | ~~**#040**~~ | ~~**Brain**~~ | ~~**Done #040.** `knowledge_ingestor.py` shipped: 113 KNOWLEDGE_DOC nodes + 40 SKILL nodes + 19 COMPANION nodes + 1,177 reference edges (DOCUMENTED_IN/SKILLED_IN/DISCOVERED_IN). Covers knowledge/domains/ (44 docs), .agents/skills/ (40 skills), session retros (34), intelligence docs, HTML companions (19). Enforced by preflight check S5 (no static artifacts when brain exists). Principle: if it has relationships, it's a node.~~ |
...
| ~~**H31**~~ | ~~**Brain v2 Phase 2: Config + Integration + Transport edges**~~ | ~~**#039**~~ | ~~**Brain**~~ | ~~**Done #040.** 50,798 nodes + 111,048 edges from 7 ingestors. Config (T042A, DMEE trees, BCM, house banks), Transports (7,745 + 46K objects â†’ 108K edges), Integration (239 RFC dests, 3,073 RFC-enabled FMs, 7 .NET apps, IDocs), SQLite schema (83 tables + 18 proven joins), Jobs (227 programs), Processes (5 E2E, 30 steps). Build time: 45s.~~ |
| ~~**H32**~~ | ~~**Brain v2 Phase 3: Process overlay + Query engine + CLI**~~ | ~~**#039**~~ | ~~**Brain**~~ | ~~**Done #040.** 4 query types operational: impact analysis (BFS with risk decay), dependency tracing (reverse BFS), structural similarity (Jaccard), gap analysis (2,931 findings on first run). CLI: `python -m brain_v2 build\|stats\|impact\|depends\|similar\|gaps\|search\|critical\|path`. Validated: HR_READ_INFOTYPE impact â†’ 9 affected objects, ZCL_HR_FIORI_EDUCATION_GRANT depends â†’ 26 dependencies.~~ |
| ~~H29~~ | ~~**Update 510 SKAT text differences P01â†’D01**~~ | ~~#034~~ | ~~Data Sync~~ | ~~**Done #038**: real scope was **1,690 rows multi-language** (not 510 English-only from #034 note). 1,511 UPDATEs (518 E + 500 F + 493 P) + 179 INSERTs (87 F + 92 P). Pattern: SELECT SINGLE + UPDATE FROM ls (avoids 72-char ABAP truncation bug). 141 batches, ok=1690 ko=0, gap=0 verified. Log: `knowledge/domains/FI/h29_skat_sync_log.md`. Side-fix: `rfc_helpers.py` RECONNECTABLE_ERRORS extended with RFC_CLOSED/broken/WSAE* patterns after mid-flight crash at batch 31.~~ |
...
| ~~H28~~ | ~~Bank Statement EBS Companion HTML~~ | ~~#029~~ | ~~Viz~~ | ~~Done #030: `bank_statement_ebs_companion.html` â€” 10 tabs (Overview, E2E Chain, Config Tiers, Posting Rules, Algorithms, GL Structure, BA Determination, Production Reality, Interactive Map, Glossary). Includes production analysis: 97% outgoing clearing, 46.5% incoming, 85.7% algo 015. SVG network diagram.~~ |
| ~~H15~~ | ~~Read Blueprint BCM pages 21-47~~ | ~~#021~~ | ~~Knowledge~~ | ~~Done #022 â€” Full 21 SAP Notes, Delegation of Authority table, grouping rules, XML char handling all extracted~~ |

### ðŸŸ¡ DESVIACIONES DE ESQUEMA EN LOS STORES â€” detectadas 2026-08-19, sin tratar

Las detecta `python Zagentexecution/quality_checks/store_schema_check.py` (enganchado al hook Stop,
sale 1). **Ninguna es de la sesiÃ³n 101**: vienen de las sesiones 51, 62, 66, 69, 71, 79, 97 y 98.
Hasta ahora nadie las veÃ­a; ahora salen en cada cierre.

**No se arreglaron a propÃ³sito.** Rellenar `created_session` en 13 claims donde el dato no existe
serÃ­a inventarlo, y es exactamente lo que la regla `feedback_label_inferred_vs_measured` prohÃ­be.

#### claims.json â€” ~~104~~ ~~76~~ **41** de 520, por combinaciÃ³n de lo que falta

| # | CuÃ¡ntos | Falta | Tratamiento |
|---|---:|---|---|
| ~~S1~~ | ~~28~~ 0 | ~~`domain_axes`~~ | **HECHO 2026-08-19.** Y NO era derivable del `domain` como escribÃ­: el mismo dominio produce hasta 36 `domain_axes` distintos. Lo resolviÃ³ el generador que ya existÃ­a, `brain_v2/scripts/backfill_domain_axes.py`, que infiere por palabra clave sobre el texto del claim. 44 claims + 49 reglas + 1 incidente al 100% |
| ~~S2~~ | ~~23~~ 0 | ~~`resolution_notes` + `resolved_session`~~ | **HECHO 2026-08-19** |
| ~~S3~~ | ~~11~~ 0 | ~~`resolved_session` + `status`~~ | **HECHO**: los 11 tenÃ­an `status='VERIFIED'`, que describe verificaciÃ³n y no ciclo de vida â€” y eso ya lo dicen `claim_type=verified_fact` + `confidence=TIER_1` en los 11 sin excepciÃ³n. Normalizado a `active` |
| ~~S4~~ | ~~9~~ 0 | ~~`resolution_notes`~~ | **HECHO 2026-08-19** |
| S5 | 8 | `created_session` + `resolved_session` + `status` | **NO automatizable**: la sesiÃ³n de origen no se puede deducir |
| S6 | 25 | combinaciones de los anteriores | Mismo criterio por campo |

#### claims.json â€” inconsistencia de TIPO, ademÃ¡s de ausencia

~~`created_session` convive en cuatro formatos~~ â€” **HECHO 2026-08-19**: normalizados **91**
referencias de sesiÃ³n a entero (`'#51'`, `'#051'`, `'s079'`, `'S-098'` â†’ `51`, `79`, `98`) en los tres
...
nÃºmeros de sesiÃ³n, y convertirlas inventarÃ­a el dato. Y 13 ausentes, que tampoco se pueden deducir.

`status` tiene 7 valores para 520 claims: `active` (448) Â· `superseded` (37) Â· ausente (21) Â·
`VERIFIED` (11) Â· y tres con un solo uso. **`VERIFIED` parece un `claim_type` colado en `status`** â€”
verificar antes de tocar.

#### incidents.json â€” ~~3~~ 0 Â· **HECHO 2026-08-19**

| Incidente | Faltaba | CÃ³mo se resolviÃ³ |
|---|---|---|
| `INC-BUDGETRATE-EQG` | `primary_object_id` | **Derivado del propio registro**: su `root_cause_summary` habla del *"15-member `ZFIX_EXCHANGERATE` composite"* y es el 2Âº de `related_objects` |
| `INC-000006906` | `root_cause_summary` | **Resumido de su doc Â§2** y citando la fuente: `MODE 'E'` en los BDC + WAN lento (`YTBAE002.abap:27`) |
| `INC-000005638` | `chain_anchor_coverage_pct`, `chain_audited_session` | **`null` a propÃ³sito.** La auditorÃ­a de cadena ocurriÃ³ o no; los demÃ¡s tienen valor porque se auditaron en las sesiones 54/56/78/99. Ã‰ste nunca. Poner un porcentaje sin haberla hecho serÃ­a inventarlo â€” queda anotado en `chain_anchor_note` |

De paso, `INC-PSTLADR-NOV2026` llevaba `chain_audited_session: 's101'` donde los demÃ¡s son entero.
Era mÃ­o, de esta misma sesiÃ³n. Normalizado a `101`.

#### feedback_rules.json â€” ~~42~~ **16** de 214
...
| Falta | CuÃ¡ntos | Tratamiento |
|---|---:|---|
| ~~`derives_from_core_principle`~~ | ~~38~~ 0 | **HECHO 2026-08-19.** LeÃ­das una a una y asignadas. Reparto final del corpus: CP-003 114 Â· CP-001 70 Â· CP-002 30 |
| `created_session` | 16 | **No deducible.** Dejar como estÃ¡ |
| `rule` | ~~2~~ 0 | **HECHO 2026-08-19.** No estaban vacÃ­as: el texto vivÃ­a bajo `statement` y bajo `title` â€” la misma divergencia que se cometiÃ³ hoy con los claims. Renombrada la clave, sin perder nada |

#### Por dÃ³nde empezar

1. ~~Las 2 reglas sin `rule`~~ â€” **hecho**. No estaban vacÃ­as, el texto estaba bajo otra clave.
2. ~~S1 (28) y la normalizaciÃ³n `'#51'` â†’ `51`~~ â€” **hechas**.
3. ~~S2 + S4 (32)~~ â€” **hechas**. Y los 6 `superseded` que parecÃ­an necesitar decisiÃ³n no la
   necesitaban: llevaban el dato bajo otra clave (`superseded_session`, o la sesiÃ³n dentro de
   `superseded_reason`). Recuperado, no inventado.
4. ~~Decidir S3~~ â€” **resuelto sin convenciÃ³n**: no habÃ­a que asumir nada.
5. **Lo que QUEDA (41)**: 27 sin `created_session`+`resolved_session`+`status` y 14 sueltos.
   Todos necesitan la sesiÃ³n de origen, que no se puede deducir. **Dejar como estÃ¡n** â€” inventar la
   procedencia es peor que no tenerla.

Re-medir: `python Zagentexecution/quality_checks/store_schema_check.py`

### ðŸŸ¢ BACKLOG â€” When blocking/high are clear
...
| G9 | **Publish `sap-intelligence` SKILL.md** to ecosystem | ecosystem | Survives â€” governance obligation to ecosystem coordinator |
| G10 | **Promote Transport Companion pattern** | ecosystem | Survives â€” ecosystem obligation |
| G11 | **Promote Company Code Copy checklist** | ecosystem | Survives â€” ecosystem obligation |
...
| ~~G16~~ | ~~Real Estate domain extraction~~ | ~~#005b~~ | ~~KILLED #036: new domain discovery â‰  requirement. No RE work in pipeline.~~ |

#### CTS Dashboard Fixes
| # | Task | First raised | Notes |
|---|------|-------------|-------|
| ~~G17~~ | ~~Improve module classification~~ | ~~#004~~ | ~~KILLED #036: 3,329/4,168 in "General IMG" â€” cosmetic. Dashboard works.~~ |
| ~~G18~~ | ~~total_mods verification~~ | ~~#004~~ | ~~KILLED #036: trivial QA on a working dashboard.~~ |
| ~~G19~~ | ~~topbar KPI sync~~ | ~~#004~~ | ~~KILLED #036: hardcoded value vs data mismatch â€” 5 min fix if ever demanded, not a PMO item.~~ |
| ~~G20~~ | ~~TADIR cache enrichment~~ | ~~#004~~ | ~~KILLED #036: SOTR/VARX skipped â€” no user-facing impact.~~ |
...
| ~~G26~~ | ~~Brain auto-refresh workflow~~ | ~~#006~~ | ~~KILLED #036: see G1/H6 â€” brain is write-only. Auto-refresh = more writes, not more value.~~ |
...
| ~~G41~~ | ~~Verify SuccessFactors EC migration status~~ | ~~#032~~ | ~~Closed #035: SF EC IS ACTIVE. ECPAO_OM_OBJECT_EXTRACTION (43 parallel jobs, 1,290 runs) + ECPAO_EMPL_EXTRACTION (3 jobs, 51 runs). Massive OM+Employee extraction to EC Payroll. Not "planned" â€” live in production.~~ |
...
| G47 | **Investigate Data Hub target system** | #035 | YFM_OUTPUT_INDIRECT_COSTS_DH + YHR_ORG_UNIT_COUNT_DH + YFM_STAFF_COST_DISTRIBUT_DH â€” "DH" suffix = Data Hub. What system is this? |

#### Future Ideas
| # | Task | First raised | Notes |
|---|------|-------------|-------|
| G28 | **Fiori PA Mass Update App** | #002 | Survives â€” consolidates PRAA* BDC (135/quarter) + absorbs G54. Real business op value. **Rejustified #037**: age 33 sessions but real consolidation value confirmed. Deadline: ship scoping doc (not app) in 5 sessions or KILL. Needs coupling with HR roadmap, not H13. |
| ~~G29~~ | ~~Coupa â†’ SAP API Interface~~ | ~~#002~~ | ~~KILLED #036: speculative. COUPA file + BDC integration is working (integration_map_complete.md). No business push to replace.~~ |
...
| ~~G35~~ | ~~Catch live BDC field data~~ | ~~#002~~ | ~~KILLED #036: requires live SAP + SM35 access + a failing session â€” too many preconditions for nonzero value.~~ |
| ~~G36~~ | ~~sap_bp_conversion SKILL.md~~ | ~~#005b~~ | ~~KILLED #036: future skill for future migration. YAGNI.~~ |
| G55 | **BP Conversion Readiness: research SAP vendorâ†’BP migration strategies** | #048 | **RESURRECTED from G14+G36 â€” now justified by INC-000006073.** Wrong KTOKK caused production failure. 21 BP tables now in Gold DB (2.5M rows). Research SAP help (help.sap.com/docs/SAP_S4HANA_ON-PREMISE vendor-customer-integration), SAP blogs, conversion strategies. Analyze UNESCO data: KTOKK distribution, CVI_VEND_LINK gaps (0 rows = no BPâ†”vendor mapping yet), BUT000 vs LFA1 coverage (559 BP vs 316K vendors = <1% converted). Output: `knowledge/domains/BusinessPartner/bp_conversion_readiness.md` |
| G56 | **Travel domain discovery: KTOKK anomalies + GGB1 coverage gaps across all company codes** | #048 | Cross LFB1.AKONT with GB901/GB922 to find every GL with vendors but no substitution rule â€” systematic INC-000006073 prevention. Also: KTOKK vs PERSG cross-check to find more wrong vendor account groups. |
...
Items completed across all 19 sessions â€” kept for audit trail.

| # | Task | Completed | Session |
|---|------|-----------|---------|
| ~~1~~ | Extract FMIFIIT full | 2026-03-15 | #005 |
| ~~2~~ | Brain redesign â€” Living Knowledge Engine | 2026-03-15 | #006 |
...
| ~~40~~ | GL + Cost Element P01â†’D01 sync (880 records, 6 tables, gap=0) | 2026-04-03 | #034 |
...
## SESSION CLOSE PROTOCOL â€” Mandatory Reconciliation
...
6. **Update MEMORY.md** pending count (just the number, not the list â€” list lives HERE)
...
1. **Every session**: Read this PMO Brain FIRST â€” it's the single source of truth
2. **Every session**: Pick 1 ðŸ”´ Blocking + 1-2 ðŸŸ¡ High Priority tasks
3. **Every session end**: MANDATORY reconciliation (see protocol above)
4. **Brain rule**: After any extraction â†’ run `sap_brain.py --build --html`
5. **Skill rule**: After discovering a new pattern 3x â†’ create a skill for it
6. **P01 rule**: All monitoring data from P01 only. Never use D01 data for decision-making


## s097 â€” PENDIENTES (recuperar en la prÃ³xima sesiÃ³n)

> Fuente de verdad: `brain_v2/capability_model/execution_backlog.json`. Esto es el Ã­ndice.
> Estado del modelo: `graph_queries.py profile` Â· `methods` Â· `coherence`.

### Bloqueado por el usuario (no por el modelo)
- **BACKUP** â€” `DURABILITY 0.10`. Golden DB 13,28 GB + `~/.claude`, sin respaldo confirmado.
  Contiene ahora `df14l`, `tadir_obj`, `tfdir_all` y la base de D01. Git no los protege.
- **EXT-HCM-TABLES** â€” 188.636 ejecuciones, cero tablas. Es dato de personal: falta confirmar
  alcance y restricciones de privacidad.
- **AN-FLOW-B2C2A** â€” falta la explicaciÃ³n de la relaciÃ³n PROJ2FUND â†” B2C2A (Â¿los fondos se
  ejecutan por proyectos? Si sÃ­, los dos flujos se fusionan y el event log se construye distinto).

### Prioridad 1
`AN-PS-DOC` (1,6M ejecuciones, sin documento) Â· `AN-PBC-DEEP` (68 usuarios, 4/11 celdas) Â·
`AN-CLAIM-VERIFY` (desde 24,5%) Â· `AN-ALGO-GOLDEN` (un fix sin caso no es un fix)

### Prioridad 2
`AN-GM-CAPABILITY` (Grants, antes del cliente #2) Â· `AN-CRP-CELLS` Â· `AN-FLOW-POST2BUDGET` Â·
`AN-FLOW-PROJ2FUND` Â· `AN-INTEGRATION-VERIFY` (794.261 ejecuciones movidas, sin verificar) Â·
`AN-EVENTLOG-DEFS` Â· `AN-ALGO-CROSSCHECK` (ascenso vs clasificador: el desacuerdo es un defecto
en uno de los dos â€” la validaciÃ³n mÃ¡s barata que tenemos, sin usar) Â· `AN-ALGO-ADAPTIVE-FEED`

### Prioridad 3
`AN-LEGACY-ARCHIVE` (8 extractores superados) Â· `AN-FAILMODE-VIEW`

### Defectos declarados de algoritmos nuevos (arreglar antes de citar sus cifras)
- **F1** trunca nombres de destino y escanea una muestra â†’ `230 DEAD` lleva error de parseo.
- **F2** por la misma truncaciÃ³n puede partir una flota en dos: las de 17 y 21 endpoints
  comparten prefijo GUID y podrÃ­an ser el mismo satÃ©lite. **No citar 40 como cifra cerrada.**
- **improve_algorithms** no pondera por PROFUNDIDAD en el DAG, y deberÃ­a: mejorar una capa baja
  levanta todo lo de arriba.


---

## Capacidad de mineria â€” pendientes abiertos (sesion 103, 2026-08-25)
...
### Prioridad 1 â€” capacidad que existe y no se puede reusar
- **MIN-VARIANT-CORPUS** â€” extraer VARI/VARID de las **127 variantes DISENADAS** (35 programas;
...
- **MIN-ACCOUNT-CLASSES** â€” extraer `FAGL_011ZC` al Gold DB y materializar
  ...
  version se elige por la VARIANTE de RFBILA00 (`BILAVERS`), nunca por T011 â€” usar la
  equivocada invento un hueco de 144 M EUR.
- **MIN-TRANSPORT** â€” 20.915 transportes en E070, **15 scripts** que los analizan y **UN** solo
...
### Prioridad 2 â€” cobertura del censo
- **MIN-REGISTER-23** â€” 23 mineros vivos sin registrar, borradores ya emitidos en
...
- **MIN-OTROS-REPOS** â€” el censo solo mira este repo. Los mineros de `FINCLOSSING`,
  `unesco-sap-brain` y `unescore20-PPM-brain` no los ve nadie desde aqui.
- **MIN-TRANSCRIPT** â€” parte del metodo se genera en la CONVERSACION y no llega a tocar disco.
  A35 mira ficheros; deberia mirar tambien el transcript de la sesion.

### Prioridad 3 â€” la capa de juicio
- **MIN-ARBITRO** â€” los choques entre mineros se SACAN a la luz (`mining_bus.py choques`) y no
...
- **MIN-PRECONDICIONES** â€” los cinco modos de fallo de la familia B dicen lo mismo â€” *si la
  precondicion esta mal, la salida es verosimil y falsa* â€” y **ninguno la comprueba**: solo la
...
`drift_signals.json` Â· `p2p_ocel2_summary.json` Â· `reality_filter.json` â€” son agregados sin
...
### Aparcado por JP durante la reparacion (sesion 103) â€” decidir despues
- **MIN-COMO-SKILL** â€” el INDICE DE CAPACIDADES (`process_mining/ask.py`: que se puede preguntar
...
- **MIN-LLAMABLE** â€” cada minero es una CAPACIDAD invocable desde otro descubrimiento o desde el
...
### â›” Capacidad de mineria â€” los TRES que quedan abiertos (sesion 103, 2026-08-25)
...
- **MIN-OBEDECER** â€” *que la memoria se LEA no es que se OBEDEZCA.* Los 32 mineros importan
  ...
  FORMA disfrazada de medida de efecto â€” el defecto que esta sesion cazo cuatro veces, cometido
  por la puerta que existe para cazarlo. Queda escrito en el `failure_mode` de A37.
  **Como se cierra:** que la `implication` sea EJECUTABLE, no solo texto â€” una comprobacion
  asociada que BLOQUEE publicar una cifra que la contradice, en vez de un aviso que se lee y se
  ignora.

- **MIN-REGISTRAR-45** â€” 45 mineros vivos detectados por el censo y sin registrar, con
  ...
  **hay que averiguarlo CORRIENDOLO**: inventarlo es peor que dejarlo vacio porque parece
  ...
  INTERPRETAR, agrupar, donde se uso) â€” registrar solo la parte contable mata el criterio, que
  es lo unico que no se puede volver a derivar.

- **MIN-MIRAR-LOS-NUEVOS** â€” ~418 nombres que el delta destapo y **nadie ha mirado**. No
